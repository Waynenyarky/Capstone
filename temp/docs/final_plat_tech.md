...
Figure 0.0 Life Cycle of Extreme Programming Diagram
...
Table x.x Software Requirements
...
Table x.x Hardware Requirements
...
2.6.1 AI Expansion: Enhanced Bilingual Classification
...
2.6.2 Blockchain Expansion: Gas Optimization and Audit History Retrieval
...
2.6.3 Integration and Additional Features
...
Security testing followed a comprehensive approach categorizing tests into NORMAL, EDGE, and ATTACK scenarios to thoroughly evaluate system resilience, as summarized in Appendix D.4. NORMAL tests verified expected functionality under standard usage patterns. EDGE tests examined system behavior with boundary conditions and unusual but valid inputs. ATTACK tests simulated malicious attempts to compromise system security.
...
Web application performance focused on response times, database query optimization, and frontend rendering efficiency. The microservices architecture enabled independent scaling of components, allowing performance optimization at the service level rather than requiring monolithic application tuning. Detailed web performance metrics are provided in Appendix E.3.

The dashboard load time consistently remains under 2 seconds as per non-functional requirements, with API response times averaging 200-500 milliseconds for database operations. Database query optimization was implemented through indexing for frequently accessed fields, and frontend optimization was achieved through lazy loading and code splitting for improved perceived performance.
...
Integration testing validated system functionality across component boundaries, ensuring seamless operation between AI, blockchain, web, and mobile components. End-to-end workflow testing confirmed proper system behavior under realistic usage scenarios, as documented in Appendix F.3.

The complete permit workflow was tested from application submission through approval, with Filipino business descriptions processed correctly by the AI classification integration. All critical actions were properly logged to the blockchain audit trail, and concurrent operations across different user roles were validated. The system achieved a 100% success rate for end-to-end permit processing, with proper inter-service authentication and data flow maintained. Data consistency was preserved across all system components, and graceful failure recovery with user notification was confirmed.
...
The first challenge involved AI test data generation, where limited Filipino business description test cases initially constrained testing coverage. This was addressed by generating a comprehensive test dataset covering various business types with 4,032 bilingual examples, resulting in robust testing coverage for Filipino language support. The second challenge involved the blockchain test environment, where Ganache network limitations affected performance testing accuracy. This was resolved by implementing local testing with simulated gas cost analysis, achieving accurate performance benchmarking despite environment constraints. The third challenge involved multi-user testing, where coordinating testing across different user roles and devices proved complex. This was addressed by developing systematic test scenarios with role-specific test cases, resulting in comprehensive validation of multi-user workflows.
...
The key achievements include all core features operating as specified, response times meeting non-functional requirements under 2 seconds for dashboard operations (Appendix E.3), zero critical security vulnerabilities identified (Appendix D.4), AI classification accuracy of 99.88% exceeding the 95% target (Appendix E.4), and stable operation under various load conditions. The comprehensive evaluation process confirmed that BizClear successfully addresses LGU business permit processing challenges while providing a solid foundation for future enhancements and scalability.
...
B.1 Sprint 2: AI Expansion Plan
Problem Statement
...
- Bilingual training data expansion with 4,032 total examples (73% Filipino, 27% English)
...
- Risk 1: Filipino language variations and regional terms
  Mitigation: Canonical token mapping handles variations; TF-IDF character n-grams capture morphological patterns
- Risk 2: Lower confidence scores for Filipino input compared to English
  Mitigation: Balanced training data ensures equal performance across languages
- Risk 3: Mixed language business descriptions
  Mitigation: Enhanced TF-IDF vectorization handles code-switching through hybrid features

Timeline
- Day 4: Expand Filipino examples in dataset, implement canonical token mapping
- Day 5: Retrain scikit-learn model with enhanced features, test Filipino language recognition
- Day 6: Comprehensive testing across NORMAL, EDGE, ATTACK scenarios, performance optimization
...
Sprint 1 Training Configuration Table

| Parameter | Value |
|-----------|-------|
| Algorithm | LinearSVC (Calibrated) |
| Training Samples | English business descriptions |
| Feature Extractor | TF-IDF Vectorization |
| LOB Categories | 80 |

Sprint 1 Functional Test Results Table

| Test Case | Input | Expected Output | Result |
|-----------|-------|-----------------|--------|
| Happy Path | "Computer Repair Shop" | Repair Services | PASSED |
| Happy Path | "Restaurant and Catering" | Food Services | PASSED |
| Edge Case | "store" | Validation error | PASSED |
| Edge Case | "Software Development" | IT / BPO services | PASSED |
| Attack Case | XSS injection attempt | Safe text processing | PASSED |

Sprint 1 validated the core ML architecture and established the foundation for bilingual expansion.
...
Sprint 2 Training Configuration Table

| Parameter | Value |
|-----------|-------|
| Algorithm | LinearSVC (Calibrated) |
| Training Samples | 7,335 (including augmentation) |
| Base Samples | 4,032 (73% Filipino, 27% English) |
| Noisy Augmented Samples | 3,218 |
| Feature Extractor | TF-IDF Hybrid (Word + Character) |
| Cross-Validation Accuracy | 99.99% |

Sprint 2 Functional Test Results Table

| Test Case | Input | Expected Output | Result |
|-----------|-------|-----------------|--------|
| Happy Path (English) | "Computer Repair Shop" | Repair Services | PASSED - 99%+ confidence |
| Happy Path (Filipino) | "tindahan ng pagkain" | Food Services | PASSED - 99%+ confidence |
| Edge Case (Mixed) | "parlor for haircut" | Personal Services | PASSED - 98%+ confidence |
| Edge Case (Minimal Filipino) | "tindahan" | Retail LOBs | PASSED |
| Attack Case | XSS injection attempt | Safe text processing | PASSED |

Sprint 2 validated the successful expansion to bilingual support with native Filipino language understanding.


E.4 AI Model Performance Evaluation

Comprehensive Model Performance Metrics

The BizClear AI business classification model underwent comprehensive evaluation using a held-out test set of 800 examples (73% Filipino, 27% English) that the model had never seen during training. The evaluation measured classification accuracy, precision, recall, and F1 scores across all 80 LOB categories.

Test Environment: Docker containerized AI service
Test Dataset: 800 held-out test examples (bilingual)
Test Method: scikit-learn evaluation metrics
Model: LinearSVC with TF-IDF hybrid word-character vectorization
...
| Test Rows | 800 |
| Number of LOB Categories | 80 |

Confidence Gate Analysis Table

| Threshold | Coverage | Precision | Samples Kept |
|-----------|----------|-----------|---------------|
| ≥0.30 | 98.1% | 99.9% | 785 |
| ≥0.50 | 92.4% | 99.9% | 739 |
| ≥0.70 | 77.5% | 100.0% | 620 |
...
The model achieved exceptional performance across all metrics, exceeding the 95% accuracy target and demonstrating robust bilingual classification capabilities.


C.2 Blockchain Prototype Test Results

C.2.1 Sprint 1: Core Audit Trail Evaluation
...
| Verify Data | PASSED | Original content → Verify → Returns verified | Content verified against blockchain |
| Tamper Detection | PASSED | Modified data detection | Original hash valid, tampered hash rejected |
| Security | PASSED | AUDITOR_ROLE enforcement | Unauthorized access blocked |
| Edge Case | PASSED | Large dataset + validation | 1000 entries verified |
| Attack Case | PASSED | Security stress testing | Re-entrancy protected, malformed inputs rejected |

Sprint 1 Gas Consumption Table

| Operation | Gas Used | Notes |
|-----------|----------|-------|
| logAuditHash (first) | 65,856 gas | Contract initialization |
| logAuditHash (subsequent) | ~65,856 gas | Stable performance |
| logAuditHash (average) | 65,856 gas | Consistent across operations |
| verifyHash | 8,800 gas | O(1) - constant regardless of dataset size |
| AccessControl deployment | 2.1 million gas | One-time cost |
| AuditLog deployment | 1.8 million gas | One-time cost |

Sprint 1 established the core audit trail functionality with tamper-evident hash logging and role-based access control.


C.2.2 Sprint 2: Gas Optimization and History Retrieval Evaluation

Test Configuration
Sprint 2 expanded blockchain capabilities with audit history retrieval functions and V3 epoch digest anchoring for gas cost optimization.

Test Environment: Ganache local blockchain network
New Functions: getAuditHistory(), getRecentAudits(), anchorDigestRoot()
Optimization Strategy: V3 Epoch Digest Anchoring

Sprint 2 New Function Test Results Table

| Function | Test Case | Status | Details |
|----------|-----------|--------|----------|
| getAuditHistory() | Time-range filtering | PASSED | Returns entries within specified dates |
| getAuditHistory() | Empty range | PASSED | Returns empty array correctly |
| getRecentAudits() | Recent N entries | PASSED | Returns last N audit entries |
| getRecentAudits() | Pagination | PASSED | Handles large datasets efficiently |
| anchorDigestRoot() | Digest anchoring | PASSED | Successfully anchors epoch digest |
| anchorDigestRoot() | Authorization | PASSED | AUDITOR_ROLE required |

V3 Gas Optimization Results Table

| Metric | Baseline (Sprint 1) | V3 Optimized (Sprint 2) | Reduction |
|--------|---------------------|-------------------------|----------|
| Monthly Gas (5K LGU) | 3.3M gas | 416K gas | 87.4% |
| Monthly Cost (Ethereum Mainnet) | ~$3,300 | ~$416 | 87.4% |

Ethereum Mainnet Cost Calculation Table

| Input | Value | Calculation |
|-------|-------|-------------|
| Gas per on-chain write | 65,856 gas | Measured from Sprint 1 tests |
| Cost per on-chain write | $0.066 | Given cost assumption |
| Monthly audit events | 50,000 events | 5,000 businesses × 10 events/month |
| V3 batch size | 100 events per digest | Design parameter |
| Sprint 2 overhead | $383.00 | Epoch management + retrieval overhead |

Ethereum Mainnet Monthly Cost Calculation Table

| Scenario | On-chain writes/month | Calculation | Monthly Cost |
|----------|------------------------|-------------|--------------|
| Sprint 1 (per-transaction logging) | 50,000 writes | 50,000 × $0.066 | $3,300 |
| Sprint 2 V3 (digest anchoring only) | 500 digests | (50,000 ÷ 100) × $0.066 | $33.00 |
| Sprint 2 V3 (total) | 500 digests + overhead | $33.00 + $383.00 | $416 |

Sprint 2 achieves ~$416/month on Ethereum mainnet through epoch digest anchoring.


D. Security Test Results

D.1 AI Security Test Results Table

| Test Category | Test Case | Input | Result | Status |
|---------------|-----------|-------|--------|--------|
| NORMAL | Filipino description | "tindahan ng pagkain" | Correct classification | PASSED |
| NORMAL | English description | "computer repair shop" | Correct classification | PASSED |
| EDGE | Empty input validation | "" | Validation error returned in <5 ms | PASSED |
| BOUNDARY | Overlength input validation | 2500-character description | Rejected (max 2000 chars enforced) | PASSED |
| ATTACK | Injection suite (XSS + SQL) | "<script>..." and "'; DROP TABLE..." | Treated as plain text, no execution | PASSED |
| DATA EXPOSURE | Prediction endpoint output filtering | Valid business description | Only LOB results returned; no model internals exposed | PASSED |

D.2 Blockchain Security Test Results Table

| Test Category | Test Case | Result | Status |
|---------------|-----------|--------|--------|
| Authorization | Non-auditor log attempt | Rejected with role error | PASSED |
| Input Validation | Zero hash attempt | Rejected with validation error | PASSED |
| Data Exposure | Event emission check | Only hash and eventType exposed | PASSED |
| Attack | Re-entrancy attempt | Protected by contract design | PASSED |
| Arithmetic Safety | Integer overflow/underflow checks | Solidity 0.8+ automatic revert protections active | PASSED |

D.3 Web Application Security Test Results Table

| Test Category | Test Case | Result | Status |
|---------------|-----------|--------|--------|
| Authentication | Invalid credential login | Rejected with unauthorized response | PASSED |
| Authentication | MFA-protected login | OTP required before token issuance | PASSED |
| Authorization | Business owner accessing officer route | Blocked by RBAC policy | PASSED |
| Authorization | Role-scoped dashboard access | Only permitted modules visible per role | PASSED |
| Input Validation | SQL injection payload in form field | Sanitized/rejected server-side | PASSED |
| Input Validation | XSS payload in text field | Escaped and treated as plain text | PASSED |
| Session Management | Expired JWT token usage | Rejected with token-expired response | PASSED |
| Transport Security | HTTPS/TLS enforcement | Secure transport required | PASSED |
| API Security | Missing X-API-Key on inter-service request | Rejected by service auth guard | PASSED |
| API Security | Valid X-API-Key on inter-service request | Accepted with authenticated service context | PASSED |

D.4 Security Testing Summary Table

| Component | Tests Passed | Tests Failed | Pass Rate |
|-----------|--------------|--------------|-----------|
| AI Service | 6 | 0 | 100% |
| Blockchain | 5 | 0 | 100% |
| Web Application | 10 | 0 | 100% |
| Total | 21 | 0 | 100% |
...
| Metric | Value |
|--------|-------|
| Cold Start Response Time | 328 ms |
| Warmed Response Time (average) | 53 ms |
| Minimum Response Time | 4.12 ms |
| Maximum Response Time | 13.65 ms |
| Average Response Time | 6.94 ms |
| Success Rate | 100% |
| Taxonomy Cache Optimization | 98.7% faster |

E.2 Blockchain Performance Metrics Table

| Metric | Value |
|--------|-------|
| logAuditHash Gas | 65,856 gas |
| verifyHash Gas | 8,800 gas (O(1)) |
| Baseline Monthly Gas (5K LGU) | 3.3M gas |
| V3 Monthly Gas (5K LGU) | 416K gas |
| Baseline Monthly Budget (Mainnet) | ~$3,300 |
| V3 Monthly Budget (Mainnet) | ~$416 |
| V3 Monthly Budget (Polygon) | ~$0.16 |

E.3 Web Application Performance Table

| Metric | Target | Achieved |
|--------|--------|----------|
| Dashboard Load Time | < 2 seconds | ✓ Met |
| API Response Time | < 500 ms | ✓ Met |
| Database Query Time | < 100 ms | ✓ Met |
...
| Blockchain Audit | 6 | 6 | 0 | 100% |
| Web Platform | 8 | 8 | 0 | 100% |
| Mobile Support | 4 | 4 | 0 | 100% |
| Total | 23 | 23 | 0 | 100% |
...
| Component | Tests | Passed | Failed | Pass Rate |
|-----------|-------|--------|--------|-----------|
| AI Filipino Support | 6 | 6 | 0 | 100% |
| Blockchain Gas Optimization | 4 | 4 | 0 | 100% |
| Integration | 5 | 5 | 0 | 100% |
| Total | 15 | 15 | 0 | 100% |

F.3 Integration Test Results Table

| Test Scenario | Components Tested | Result |
|---------------|-------------------|--------|
| Complete Permit Workflow | Web + AI + Blockchain | PASSED |
| Filipino Classification | AI + Web | PASSED |
| Audit Trail Logging | Blockchain + Web | PASSED |
| Multi-User Operations | Web + Auth | PASSED |
| Mobile Inspection | Mobile + Web | PASSED |


G. Screenshots of the System
