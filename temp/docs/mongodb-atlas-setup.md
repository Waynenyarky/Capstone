...
### Option A: Use Atlas with Docker (recommended)
...
2. Start the app **without** the local MongoDB container. Either use the start script with the `--atlas` flag (recommended), or run compose directly:
   ```bash
   ./start.sh --atlas
   ```
   ...
   ```
   This uses `MONGO_URI` from `.env` and does not start the local `mongodb` service. You can combine flags: `./start.sh --atlas --dev`, `./start.sh --atlas --skip-ipfs`, etc.

3. If you use **backend scripts** (e.g. seed, backup) that expect a local MongoDB, run them with the same `MONGO_URI` in `.env`; for backup/restore, Atlas has its own backup or you can use `mongodump`/`mongorestore` against the Atlas URI.

### Option B: Switch back to local MongoDB

- Remove or comment out `MONGO_URI` in `.env` (so the app uses the default local URI from compose).
...
## 8. Verify the app is using Atlas

1. After starting with the Atlas override, open the app and log in or sign up.
