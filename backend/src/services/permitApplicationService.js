...
class PermitApplicationService {
  ...
  async getApplicationById(applicationId, businessId = null) {
    ...
    return {
      ...
      // Documents
      documents: business.lguDocuments || {},
      // AI Validation
  ...
  async startReview(applicationId, businessId, officerId) {
    // Get officer info
    const officer = await User.findById(officerId).populate('role').lean()
    if (!officer) {
    ...
    const hashInput = Object.values(hashableData).join('|')
  ...
  async reviewApplication(applicationId, businessId, officerId, decision, comments, rejectionReason = null) {
    ...
    if (!validTransitions[oldStatus] || !validTransitions[oldStatus].includes(
      ...
      throw new Error(`Invalid status transition from ${oldStatus} to ${decision}`)
    }
    ...
    profile.businesses[businessIndex].reviewedBy = officerId
    profile.businesses[businessIndex].reviewedAt = new Date()
    profile.businesses[businessIndex].reviewComments = comments
    if (rejectionReason) {
