 ...
 */
const { encrypt, encryptDeterministic, decrypt, isEncrypted } = require('./fieldCipher')
...
function encryptionPlugin(schema, options = {}) {
  ...
  if (keyAvailable()) {
    ...
    for (const p of encryptedTopPaths) {
      ...
      if (schemaPath && schemaPath.validators) {
        schemaPath.validators = schemaPath.validators.filter(v =>
          v.type !== 'enum' && v.type !== 'regexp'
        )
      }
    }
    ...
    for (const arrPath of arrayPaths) {
      ...
      const subSchema = schemaType?.schema || schemaType?.$embeddedSchemaType?.schema
      if (subSchema) {
        subSchema.eachPath((subPath, subSchemaType) => {
          if (subSchemaType.instance === 'String' && subSchemaType.validators) {
            subSchemaType.validators = subSchemaType.validators.filter(v =>
              v.type !== 'enum' && v.type !== 'regexp'
            )
          }
        })
  ...
  function encryptFilter(filter) {
    ...
    for (const f of deterministicFields) {
      if (typeof filter[f] === 'string' && filter[f] !== '' && !isEncrypted(filter[f])) {
        filter[f] = encryptDeterministic(filter[f])
      }
      // Handle $in operator: { email: { $in: ['a@b.com', 'c@d.com'] } }
      if (filter[f] && typeof filter[f] === 'object' && Array.isArray(filter[f].$in)) {
        filter[f].$in = filter[f].$in.map(v =>
          typeof v === 'string' && v !== '' && !isEncrypted(v) ? encryptDeterministic(v) : v
        )
      }
    }
  }
  ...
  function encryptNested(obj, basePath) {
    ...
    for (const key of Object.keys(nested)) {
      if (typeof nested[key] === 'string' && nested[key] !== '' && !isEncrypted(nested[key])) {
        nested[key] = encrypt(nested[key])
      } else if (nested[key] && typeof nested[key] === 'object' && !Array.isArray(nested[key])) {
        // Recurse one more level for nested objects like address.street
        for (const subKey of Object.keys(nested[key])) {
          if (typeof nested[key][subKey] === 'string' && nested[key][subKey] !== '' && !isEncrypted(nested[key][subKey])) {
      ...
      for (const key of Object.keys(item)) {
        ...
        if (typeof val === 'string' && val !== '' && !isEncrypted(val)) {
          ...
          for (const subKey of Object.keys(val)) {
            if (typeof val[subKey] === 'string' && val[subKey] !== '' && !isEncrypted(val[subKey])) {
  ...
  function encryptUpdate(update) {
    ...
    if (update.$set) {
      ...
      for (const p of nestedPaths) {
        ...
        for (const key of Object.keys(update.$set)) {
          if (key.startsWith(p + '.') && typeof update.$set[key] === 'string' && update.$set[key] !== '' && !isEncrypted(update.$set[key])) {
    ...
    for (const f of fields) {
      if (update[f] !== undefined && !f.startsWith('$')) encryptField(update, f, false)
    }
    for (const f of deterministicFields) {
      if (update[f] !== undefined && !f.startsWith('$')) encryptField(update, f, true)
    }
  }
  ...
  function decryptNested(obj, basePath) {
    ...
    for (const key of Object.keys(nested)) {
      if (typeof nested[key] === 'string' && isEncrypted(nested[key])) {
        try { nested[key] = decrypt(nested[key]) } catch (_) {}
      } else if (nested[key] && typeof nested[key] === 'object' && !Array.isArray(nested[key])) {
        for (const subKey of Object.keys(nested[key])) {
          if (typeof nested[key][subKey] === 'string' && isEncrypted(nested[key][subKey])) {
            try { nested[key][subKey] = decrypt(nested[key][subKey]) } catch (_) {}
          }
        }
      }
    }
    ...
    for (const item of arr) {
      ...
      for (const key of Object.keys(item)) {
        ...
        if (typeof val === 'string' && isEncrypted(val)) {
          try { item[key] = decrypt(val) } catch (_) {}
        } else if (val && typeof val === 'object' && !Array.isArray(val)) {
          for (const subKey of Object.keys(val)) {
            if (typeof val[subKey] === 'string' && isEncrypted(val[subKey])) {
              try { val[subKey] = decrypt(val[subKey]) } catch (_) {}
            }
          }
        }
      }
  ...
  function decryptMixed(obj, path) {
    ...
    if (val == null) return
    if (typeof val === 'string' && isEncrypted(val)) {
      try { obj[path] = decrypt(val) } catch (_) {}
      return
    }
    if (typeof val === 'object') {
  ...
  function decryptObjectDeep(obj) {
    ...
    if (Array.isArray(obj)) {
      for (let i = 0; i < obj.length; i++) {
        if (typeof obj[i] === 'string' && isEncrypted(obj[i])) {
          try { obj[i] = decrypt(obj[i]) } catch (_) {}
        } else if (typeof obj[i] === 'object') {
    ...
    for (const key of Object.keys(obj)) {
      if (typeof obj[key] === 'string' && isEncrypted(obj[key])) {
        try { obj[key] = decrypt(obj[key]) } catch (_) {}
      } else if (typeof obj[key] === 'object') {
