...
function getScanner() {
  ...
  try {
    ...
    console.warn(
      "[fileScan] clamdjs not installed or config invalid:",
      err.message,
    );
    return null;
  }
...
function isScanConfigured() {
  return Boolean(
    process.env.CLAMAV_HOST && String(process.env.CLAMAV_HOST).trim(),
  );
}
...
async function scanFile(filePath) {
  ...
  try {
    const timeout = Math.min(
      30000,
      Number(process.env.CLAMAV_SCAN_TIMEOUT_MS) || 10000,
    );
