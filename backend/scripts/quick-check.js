...
async function quickCheck() {
  const mongoUri =
    ...
    "mongodb+srv://capstone_app:g95fxnwa1wPDdyfA@capstone.efa2aqu.mongodb.net/capstone_project?appName=capstone";
  ...
  try {
    ...
    const sampleOwner = await db
      .collection("users")
      .findOne({ "role.slug": "business_owner" })
      .catch(() => null);
    if (sampleOwner) {
      ...
      { $unwind: "$businesses" },
      {
        $match: {
          "businesses.applicationStatus": {
            $in: ["submitted", "under_review", "resubmit"],
          },
        },
      },
      { $count: "total" },
