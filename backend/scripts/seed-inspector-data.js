...
async function main() {
  ...
  if (!uri) {
    console.error(
      "Error: MONGO_URI or MONGODB_URI not set. Add to .env or set in environment.",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (!inspectorRole) {
      inspectorRole = await Role.create({
        name: "LGU Inspector",
        slug: "inspector",
      });
    ...
    const inspectorEmail =
      process.env.DEV_EMAIL_INSPECTOR || "inspector@example.com";
    ...
    const allWithEmail = await User.find({ email: inspectorEmail })
      .populate("role")
      .sort({ _id: 1 })
      .lean();
    if (allWithEmail.length > 1) {
      ...
      console.log(
        `Removing ${duplicateIds.length} duplicate user(s) for ${inspectorEmail} so one account is used for login and seed.`,
      );
    ...
    let inspector = await User.findOne({ email: inspectorEmail }).populate(
      "role",
    );
    if (!inspector) {
      ...
      await inspector.populate("role");
      console.log(
        `Corrected ${inspectorEmail}: inspector role, isStaff=true, password=${inspectorPassword}`,
      );
    }
    ...
    let officer = await User.findOne({ email: "officer@example.com" }).populate(
      "role",
    );
    if (!officer) {
      ...
      console.log(
        "Created officer@example.com (for assignedBy on inspections)",
      );
    }
    ...
    const inspectResult = await Inspection.updateMany(
      {
        businessId: { $regex: /^biz_seed_/ },
        inspectorId: { $ne: inspectorId },
      },
      { $set: { inspectorId } },
    );
    if (inspectResult.modifiedCount > 0) {
      console.log(
        `  Reassigned ${inspectResult.modifiedCount} seed inspections to waynenrq@gmail.com (_id: ${inspectorId})`,
      );
    }
    const vioResult = await Violation.updateMany(
      {
        violationId: { $regex: /^VIO-SEED-/ },
        inspectorId: { $ne: inspectorId },
      },
      { $set: { inspectorId } },
    );
    if (vioResult.modifiedCount > 0) {
      console.log(
        `  Reassigned ${vioResult.modifiedCount} seed violations to waynenrq@gmail.com`,
      );
    }
    const notifResult = await Notification.updateMany(
      {
        userId: { $ne: inspectorId },
        message: { $regex: /Seed Cafe|Seed Retail/ },
      },
      { $set: { userId: inspectorId } },
    );
    if (notifResult.modifiedCount > 0) {
      console.log(
        `  Reassigned ${notifResult.modifiedCount} seed notifications to waynenrq@gmail.com`,
      );
    }
    ...
    const notifications = [
      await Notification.create({
        ...
        message:
          "You have been assigned to inspect Seed Cafe & Restaurant today.",
      ...
      await Notification.create({
        ...
        message:
          "Inspection for Seed Retail Store has been scheduled for tomorrow.",
        relatedEntityType: "inspection",
      ...
      await Notification.create({
        ...
        message:
          "The inspection for Seed Cafe has been rescheduled to next week.",
        relatedEntityType: "inspection",
      ...
      await Notification.create({
        ...
        message:
          "Seed Cafe & Restaurant – initial inspection scheduled for today.",
        relatedEntityType: "inspection",
    ...
    console.log("Summary:");
    console.log(
      `  Inspections: ${inspections.length} (today: ${todayCount}, pending: ${pendingCount}, completed: ${completedCount})`,
    );
    ...
    console.log(
      "  Inspector _id (JWT sub when logged in):",
      String(inspectorId),
    );
    console.log("");
    console.log("Test checklist – verify each Inspector screen:");
    console.log(
      "  1. Dashboard     – Today, Pending, Completed counts (non-zero)",
    );
    ...
    console.log(
      "  5. Violations   – List of issued violations (open/resolved)",
    );
    console.log(
      "  6. Notifications – List of assignment/schedule/appeal notifications",
    );
    console.log("");
    ...
    const verifyNotif = await Notification.countDocuments({
      userId: inspectorId,
    });
    if (verifyInspect === 0 || verifyVio === 0 || verifyNotif === 0) {
      console.log("");
      console.log("WARNING: Verification counts after seed:", {
        inspections: verifyInspect,
        violations: verifyVio,
        notifications: verifyNotif,
      });
      console.log(
        "Backend must use the SAME MongoDB (same MONGO_URI in backend/.env).",
      );
    }
    console.log("");
    console.log(
      "If app still empty: backend now resolves waynenrq@gmail.com by email – restart backend, log out and log in again.",
    );
    console.log(
      "BASE_URL (mobile/app/.env): Android emulator http://10.0.2.2:3000 | Device http://YOUR_PC_IP:3000",
    );
  } catch (err) {
    console.error("Error:", err.message);
    if (err.code === 11000) {
      console.error(
        "Duplicate key - try running with --reset to clear existing data first",
      );
    }
    process.exit(1);
  } finally {
