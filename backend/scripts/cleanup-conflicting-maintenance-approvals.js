...
async function run() {
  ...
  try {
    ...
    for (const approval of approvals) {
      ...
      if (
        keepIds.has(approval.approvalId) ||
        cancelIds.has(approval.approvalId)
      )
        continue;
      ...
      for (const other of approvals) {
        ...
        if (keepIds.has(other.approvalId) || cancelIds.has(other.approvalId))
          continue;
    ...
    const result = await AdminApproval.updateMany(
      {
        approvalId: { $in: Array.from(cancelIds) },
        status: { $in: ["pending", "approved"] },
      },
      {
        $set: {
    ...
    console.log(
      `Updated ${result.modifiedCount || 0} approval(s) to cancelled`,
    );
  } finally {
