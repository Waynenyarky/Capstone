...
async function checkDatabase() {
  ...
  try {
    ...
    businessOwners.forEach((owner, i) => {
      console.log(`  ${i + 1}. ${owner.firstName} ${owner.lastName} - ${owner.email} - Created: ${owner.createdAt?.toISOString()}`);
    });
    ...
    if (submittedApps.length > 0) {
      ...
      submittedApps.slice(0, 5).forEach((app, i) => {
        console.log(`  ${i + 1}. ${app.businessName || 'No Name'} - ${app.applicationStatus}`);
    ...
    if (noStatusApps.length > 0) {
      ...
      noStatusApps.slice(0, 3).forEach(app => {
        console.log(`  - ${app.businessName || 'No Name'} (ID: ${app.businessId})`);
      });
    }
    ...
    if (recentApps.length > 0) {
      ...
      recentApps.forEach((app, i) => {
        console.log(`  ${i + 1}. ${app.businessName || 'No Name'} - ${app.applicationStatus || 'no-status'} - ${app.createdAt?.toISOString()}`);
      });
    }
