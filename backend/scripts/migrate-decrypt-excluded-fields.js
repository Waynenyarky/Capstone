...
const { decrypt, isEncrypted, encryptDeterministic } = require('../shared/lib/fieldCipher')

const MONGO_URI = process.env.MONGO_URI || process.env.MONGODB_URI || 'mongodb://localhost:27017/capstone'
...
const DETERMINISTIC_MIGRATIONS = [
  { collection: 'appeals', fields: ['businessId', 'applicationId', 'violationId', 'inspectionId'] },
...
async function run() {
  ...
  for (const profile of profiles) {
    if (!Array.isArray(profile.businesses) || profile.businesses.length === 0) continue
    let modified = false
      ...
      for (const field of EXCLUDED_BUSINESS_FIELDS) {
        ...
        if (typeof val === 'string' && isEncrypted(val)) {
          try {
            ...
            console.warn(`  Failed to decrypt ${field} in profile ${profile._id}:`, err.message)
          }
        }
      }
    }
  ...
  console.log(`  Profiles updated: ${profilesFixed}, fields decrypted: ${fieldsDecrypted}`)
      ...
      for (const field of fields) {
        ...
        if (isEncrypted(val)) {
          ...
          try {
            ...
            console.warn(`  Failed to re-encrypt ${field} in ${collection} doc ${doc._id}:`, err.message)
          }
        } else {
