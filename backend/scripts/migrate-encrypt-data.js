...
const { encrypt, encryptDeterministic, isEncrypted } = require('../shared/lib/fieldCipher')
...
const ENCRYPTION_MAP = {
  users: {
    fields: [
      ...
      'office', 'passwordHash', 'mfaSecret', 'authProvider', 'providerId',
      'tokenFprint', 'placeOfBirth', 'nationality', 'fatherName', 'motherName',
      'distinctiveMark', 'profileHash', 'profileIpfsCid', 'userEthereumAddress',
  ...
  appeals: {
    fields: ['businessId', 'applicationId', 'description', 'resolution', 'violationId', 'inspectionId'],
    deterministicFields: [],
    nestedPaths: [],
    arrayPaths: [],
    mixedPaths: [],
  ...
  inspections: {
    fields: ['businessId', 'notes', 'complaintDetails', 'revokedReason', 'gpsMismatchReason', 'blockchainHash'],
    deterministicFields: [],
  ...
  permits: {
    fields: ['businessName', 'ownerName', 'address', 'lineOfBusiness', 'qrCode', 'suspensionReason', 'revocationReason'],
  ...
  postrequirements: {
    fields: ['businessId', 'permitId', 'description', 'documentUrl', 'verificationNotes'],
    deterministicFields: [],
    nestedPaths: [],
  ...
  occupationalpermits: {
    fields: ['firstName', 'lastName', 'education', 'businessPlateNo', 'employer', 'company', 'position', 'rejectionReason'],
    deterministicFields: [],
  ...
  for (const key of Object.keys(obj)) {
    if (typeof obj[key] === 'string' && obj[key] !== '' && !isEncrypted(obj[key])) {
      ...
      for (const sub of Object.keys(obj[key])) {
        if (typeof obj[key][sub] === 'string' && obj[key][sub] !== '' && !isEncrypted(obj[key][sub])) {
    ...
    for (const key of Object.keys(item)) {
      ...
      if (typeof val === 'string' && val !== '' && !isEncrypted(val)) {
        ...
        for (const sub of Object.keys(val)) {
          if (typeof val[sub] === 'string' && val[sub] !== '' && !isEncrypted(val[sub])) {
...
async function migrateCollection(db, collectionName, config) {
  ...
  console.log(`  ✅ ${collectionName}: encrypted ${updated}/${count} documents`)
  return updated
}
...
async function main() {
  ...
  console.log('======================================')
  console.log(`✅ Migration complete. Total documents encrypted: ${totalUpdated}`)
