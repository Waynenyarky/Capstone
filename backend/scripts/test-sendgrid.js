...
async function main() {
  ...
  const mailerPath = path.resolve(
    __dirname,
    "../services/auth-service/src/lib/mailer.js",
  );
  ...
  if (result && result.success) {
    ...
    console.log(
      "Check the inbox for",
      to,
      '(subject: "Email test – BizClear", code:',
      testCode + ")",
    );
    if (to.includes("mailinator.com")) {
      console.log(
        "Mailinator: https://www.mailinator.com → enter the part before @",
      );
    }
    console.log("");
    process.exit(0);
  ...
  console.error(
    "  • Resend: use DEFAULT_FROM_EMAIL=onboarding@resend.dev (no domain needed); verify API key at https://resend.com/api-keys",
  );
  console.error(
    "  • SendGrid: verify DEFAULT_FROM_EMAIL in SendGrid → Sender Authentication",
  );
  console.error(
    "  • Check EMAIL_API_KEY is valid for your provider (re_ for Resend, SG. for SendGrid)",
  );
  console.error(
    '  • See README "Verification emails (signup / business owner registration)"',
  );
  console.error("");
  process.exit(1);
}
