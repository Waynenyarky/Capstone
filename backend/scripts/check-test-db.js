...
async function checkTestDB() {
  try {
    await mongoose.connect('mongodb+srv://capstone_app:g95fxnwa1wPDdyfA@capstone.efa2aqu.mongodb.net/test?appName=capstone');
    console.log('Connected to: test database');
    
    const db = mongoose.connection.db;
    
    // Business owners
    const owners = await db.collection('users').find({ 'role.slug': 'business_owner' }).toArray();
    ...
    // Applications
    const apps = await db.collection('businessprofiles').aggregate([
      { $unwind: '$businesses' },
      { $project: {
