...
async function findDuplicatePayments(businessId) {
  ...
  for (const [key, group] of Object.entries(groups)) {
    if (group.length > 1) {
      console.log(`\nDuplicate group: ${key}`);
      console.log(
        `  - Keeping: ${group[0]._id} (created: ${group[0].createdAt})`,
      );
      toKeep.push(group[0]);

      for (let i = 1; i < group.length; i++) {
        console.log(
          `  - DUPLICATE: ${group[i]._id} (created: ${group[i].createdAt})`,
        );
...
async function main() {
  ...
  if (!businessId && !listAll) {
    console.log(
      "Usage: node scripts/clear-duplicate-payments.js <businessId> [--execute] [--list-all]",
    );
    console.log("");
    console.log("Options:");
    console.log(
      "  --execute    Actually delete duplicates (default is dry run)",
    );
  ...
  try {
    ...
    if (listAll) {
      ...
      for (const [bid, payments] of Object.entries(businessGroups)) {
        ...
        const dupCount = Object.values(groups)
          .filter((g) => g.length > 1)
          .reduce((sum, g) => sum + g.length - 1, 0);
        if (dupCount > 0) {
          console.log(
            `  ${bid}: ${payments.length} payments, ${dupCount} duplicates`,
          );
        }
      }
      ...
      const { duplicates, toKeep, total } =
        await findDuplicatePayments(businessId);
      ...
      if (duplicates.length > 0) {
        const totalDupAmount = duplicates.reduce(
          (sum, d) => sum + (d.amount || 0),
          0,
        );
        console.log(
          `Total duplicate amount: ₱${totalDupAmount.toLocaleString()}`,
        );
      }
