...
jest.mock('../../../services/business-service/src/lib/helpRequestMailer', () => {
  const actual = jest.requireActual('../../../services/business-service/src/lib/helpRequestMailer')
  return {
    ...actual,
    sendEmailFn: jest.fn().mockResolvedValue({ messageId: 'mock-id', accepted: ['test@example.com'], rejected: [] }),
  }
})
...
describe('Help Request Mailer Unit Tests', () => {
  ...
  describe('sendOfficerReplyNotification', () => {
    ...
    it('should handle email sending errors gracefully', async () => {
      ...
      await expect(sendOfficerReplyNotification(request, message)).resolves.not.toThrow()
    })
  })
  ...
  describe('sendRequestClosedNotification', () => {
    ...
    it('should handle email sending errors gracefully', async () => {
      ...
      await expect(sendRequestClosedNotification(request)).resolves.not.toThrow()
    })
  })
  ...
  describe('sendRequestInvalidNotification', () => {
    ...
    it('should handle email sending errors gracefully', async () => {
      ...
      await expect(sendRequestInvalidNotification(request)).resolves.not.toThrow()
    })
  })
})
