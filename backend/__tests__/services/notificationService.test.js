...
describe("Notification Service", () => {
  ...
  describe("markAsRead()", () => {
    ...
    it("should handle database errors gracefully", async () => {
      ...
      Notification.findOneAndUpdate = jest
        .fn()
        .mockRejectedValue(new Error("Database error"));
  ...
  describe("deleteNotification()", () => {
    ...
    it("should throw error when trying to delete another user's notification", async () => {
      await expect(
        ...
        "Failed to delete notification: Notification not found or access denied",
      );
    });
  });
  ...
  describe("Data Consistency", () => {
    ...
    it("should maintain read/unread state consistency", async () => {
      ...
      const unreadNotifications =
        await notificationService.getUserNotifications(testUser._id, {
          unreadOnly: true,
        });
      expect(unreadNotifications.notifications).toHaveLength(1);
      expect(unreadNotifications.notifications[0]._id.toString()).toBe(
        notif2._id.toString(),
      );
    });
  });
