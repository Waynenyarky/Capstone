...
beforeAll(async () => {
  ...
  const adminMongoose =
    require("../../../services/admin-service/src/models/AuditLog").base;
...
describe("GET /api/admin/permit-forms", () => {
  it("returns empty section for new setup (public)", async () => {
    const res = await request(app).get("/api/admin/permit-forms").expect(200);
...
describe("GET /api/admin/permit-forms/audit", () => {
  ...
  it("rejects unauthenticated requests", async () => {
    await request(app).get("/api/admin/permit-forms/audit").expect(401);
  });
});
