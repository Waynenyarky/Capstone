...
const calculateAuditHash =
  require("../../../services/auth-service/src/lib/auditLogger").calculateAuditHash;
...
describe("Tamper response flow", () => {
  ...
  describe("verifyAuditIntegrity job", () => {
    ...
    test("records tamper incident when hash mismatch is detected", async () => {
      ...
      const tamperEvent = securityEvents.find(
        (e) => String(e.metadata?.auditLogId) === String(log._id),
      );
  ...
  describe("Tamper incidents admin API", () => {
    test("POST /incidents with eventType audit_tamper_detected creates incident and dedupes", async () => {
      ...
      const count = await TamperIncident.countDocuments({
        auditLogIds: { $in: [auditLogId] },
        status: { $ne: "resolved" },
      });
      expect(count).toBe(1);
    });
