...
function createStepUpToken(userId) {
  ...
  return jwt.sign({ sub: String(userId), stepUp: true }, secret, { expiresIn: '10m' })
}
...
describe('Maintenance endpoints', () => {
  ...
  describe('Status Display Logic', () => {
    it('should distinguish approved-upcoming from approved-past', async () => {
      ...
      expect(upcomingApproval.requestDetails.scheduledStartAt.getTime()).toBeGreaterThan(Date.now())

      // Verify past has past scheduled start
      expect(pastApproval.requestDetails.scheduledStartAt.getTime()).toBeLessThan(Date.now())
    })
  ...
  describe('Audit Log Creation', () => {
    it('should create audit log for maintenance request', async () => {
      ...
      const approval = await AdminApproval.findOne({ approvalId: 'MAINT-AUDIT-001' })
    ...
    it('should create audit log for cancellation request', async () => {
      ...
      expect(approval.requestDetails.cancelTargetApprovalId).toBe('MAINT-TARGET')
    })
    ...
    it('should track approval actions in approvals array', async () => {
      const approval = await AdminApproval.create({
        ...
        requiredApprovals: 2,
        approvals: [
          { adminId: admin._id, approved: true, comment: 'Approved', timestamp: new Date() },
          { adminId: admin2._id, approved: true, comment: 'Approved', timestamp: new Date() },
        ],
      })
  ...
  describe('Maintenance announcement behavior', () => {
    it('creates an announcement for upcoming maintenance even when another session exists', async () => {
      ...
      const initialApproval = await AdminApproval.findOne({ approvalId }).lean()
      ...
      const createdAnnouncement = announcementsInDb.find((item) => item?.metadata?.maintenanceApprovalId === approvalId)
      ...
      const announcements = Array.isArray(announcementsResponse.body) ? announcementsResponse.body : []
      const maintenanceAnnouncement = announcements.find((item) => item?.metadata?.maintenanceApprovalId === approvalId)
      ...
      expect(new Date(maintenanceAnnouncement.expiresAt).getTime()).toBe(futureEnd.getTime())
      expect(new Date(maintenanceAnnouncement.metadata.scheduledStartAt).getTime()).toBe(futureStart.getTime())
      expect(new Date(maintenanceAnnouncement.metadata.expectedResumeAt).getTime()).toBe(futureEnd.getTime())
      expect(maintenanceAnnouncement.body).toContain('System maintenance is scheduled')
    })
  })
})
