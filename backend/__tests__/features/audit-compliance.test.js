...
const {
  ...
  maskPhoneNumber,
  maskIdNumber,
} = require("../../services/auth-service/src/lib/dataMasker");
...
async function ensureDatabaseReady() {
  ...
  if (mongoose.connection.readyState !== 1) {
    throw new Error(
      `Database connection not ready after ${maxRetries} retries`,
    );
  }
...
describe("Phase 4: Audit & Compliance", () => {
  ...
  describe("1. Audit History Endpoints", () => {
    ...
    it("should filter audit history by event type", async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(
        response.body.logs.every((log) => log.eventType === "email_change"),
      ).toBe(true);
    });
    ...
    it("should mask sensitive data in audit logs", async () => {
      ...
      const passwordLog = response.body.logs.find(
        (log) => log.fieldChanged === "password",
      );
      if (passwordLog) {
  ...
  describe("3. Compliance Logging", () => {
    ...
    it("should log audit history view", async () => {
      ...
      expect(viewLog.viewedUserId.toString()).toBe(
  ...
  describe("7. Integration: Audit History with Filtering", () => {
    ...
    it("should filter by date range", async () => {
      ...
      const response = await request(app)
        .get(
          `/api/auth/audit/history?startDate=${startDate}&endDate=${endDate}`,
        )
        .set("Authorization", `Bearer ${businessOwnerToken}`);
