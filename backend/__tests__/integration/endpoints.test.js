// Note: Email is now REST API, not nodemailer - mock is not needed but kept for compatibility
jest.mock("axios", () => ({
...
function collectRoutesFromStack(stack, prefix) {
  ...
  for (const layer of stack || []) {
    ...
    if (layer.name === "router" && layer.handle.stack) {
      ...
      routes.push(
        ...collectRoutesFromStack(layer.handle.stack, joinPaths(prefix, mount)),
      );
    }
  }
  return routes;
...
describe("API endpoint smoke tests", () => {
  ...
  beforeAll(async () => {
    ...
    let admin = await User.findOne({ email: "admin@example.com" }).populate(
      "role",
    );
    ...
    if (!business) {
      const boRole =
        (await Role.findOne({ slug: "business_owner" })) ||
      ...
      business = await User.create({
        ...
        lastName: "Business",
        email: "business@example.com",
        phoneNumber: `__unset__${Date.now()}_business`, // Unique phone number
        passwordHash: await bcrypt.hash("password123", 10),
        termsAccepted: true,
      });
  ...
  it("responds for every registered route (no 500, not default Express 404)", async () => {
    const routes = uniqRoutes(
      collectRoutesFromStack(app._router && app._router.stack, ""),
    ...
    for (const r of routes) {
      ...
      try {
        ...
        const e = new Error(
          `Request failed for ${details}: ${err && err.message ? err.message : String(err)}`,
        );
        e.cause = err;
        throw e;
      }
      ...
      if (res.status === 500 || isDefaultExpress404(res)) {
        ...
        console.error(
          `Endpoint check failed: ${r.method.toUpperCase()} ${path} -> ${res.status} ${res.text || ""}`,
        );
      }
