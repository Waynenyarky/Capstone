...
} = require("../helpers/blockchain-mocks");
...
describe("User Registry → Blockchain Integration", function () {
  ...
  beforeAll(async function () {
    ...
    testProfileHash = crypto
      .createHash("sha256")
      .update(JSON.stringify({ userId: testUserId, email: "test@example.com" }))
      .digest("hex");
  });
  ...
  describe("User Registration", function () {
    ...
    it("should retrieve user profile hash from blockchain", async function () {
      ...
      expect(result.userAddress.toLowerCase()).to.equal(
        testUserAddress.toLowerCase(),
      );
      expect(result).to.have.property("profileHash");
      expect(result.profileHash).to.equal(
        `0x${testProfileHash.padStart(64, "0")}`,
      );
    });
    ...
    it("should get userId by Ethereum address", async function () {
      const result =
        await userRegistryService.getUserIdByAddress(testUserAddress);
  ...
  describe("Profile Hash Update", function () {
    it("should update profile hash for existing user", async function () {
      const newProfileHash = crypto
        .createHash("sha256")
        .update(JSON.stringify({ userId: testUserId, updated: true }))
        .digest("hex");

      const result = await userRegistryService.updateProfileHash(
        testUserId,
        newProfileHash,
      );
  ...
  describe("Profile Document Storage", function () {
    ...
    it("should retrieve profile document from blockchain", async function () {
      const result =
        await documentStorageService.getDocument(testProfileIpfsCid);
  ...
  describe("End-to-End User Migration Flow", function () {
    it("should complete full user migration flow", async function () {
      ...
      const docResult = await documentStorageService.getDocument(
        ipfsResult.cid,
      );
