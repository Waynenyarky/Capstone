...
const OfficeHours = require("../../services/auth-service/src/models/OfficeHours");
...
describe("Account Recovery, Deletion & Session Management", () => {
  ...
  beforeAll(async () => {
    ...
    let staffRole = await Role.findOne({ slug: "lgu_officer" });
    if (!staffRole) {
  ...
  describe("1. Business Owner Password Recovery", () => {
    ...
    test("should change password with validation and session invalidation", async () => {
      ...
      expect(updatedUser.tokenVersion).toBeGreaterThan(
        businessOwner.tokenVersion || 0,
      );
    ...
    test("should reject weak password", async () => {
      ...
      expect(["weak_password", "validation_error"]).toContain(
        res.body.error.code,
      );
    });
    ...
    test("should reject password in history", async () => {
      ...
      expect(["password_in_history", "invalid_reset_token"]).toContain(
        res.body.error.code,
      );
    });
  });
  ...
  describe("2. Staff Recovery Request Flow", () => {
    ...
    test("should allow admin to issue temporary credentials", async () => {
      ...
      const tempCred = await TemporaryCredential.findById(
        res.body.temporaryCredentialId,
      );
    ...
    test("should allow admin to deny recovery request", async () => {
      ...
      const updatedRequest = await RecoveryRequest.findById(
        recoveryRequest._id,
      );
  ...
  describe("3. Account Deletion - Business Owner", () => {
    test("should require legal acknowledgment for deletion", async () => {
      ...
      const deleteRequest = await createVerifiedDeleteRequest(
      ...
      expect(res.status).toBe(400);
      expect(["legal_acknowledgment_required", "validation_error"]).toContain(
        res.body.error.code,
      );
    });
    ...
    test("should allow undoing deletion with token", async () => {
      ...
      user.deletionUndoExpiresAt = new Date(
  ...
  describe("7. Office Hours Validation", () => {
    test("should validate office hours", async () => {
      ...
      const {
        isWithinOfficeHours,
      } = require("../../services/auth-service/src/lib/officeHoursValidator");
  ...
  describe("8. High-Privilege Task Checking", () => {
    test("should check for high-privilege tasks", async () => {
      ...
      } = require("../../services/auth-service/src/lib/highPrivilegeTaskChecker");
  ...
  describe("9. IP Tracking", () => {
    test("should track IP addresses", async () => {
      const {
        trackIP,
        getRecentIPs,
      } = require("../../services/auth-service/src/lib/ipTracker");
