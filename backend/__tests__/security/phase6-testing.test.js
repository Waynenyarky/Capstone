...
describe("Phase 6: Testing - Security & Performance", () => {
  ...
  beforeAll(async () => {
    ...
    const businessOwnerWithRole = await User.findById(businessOwner._id)
      .populate("role")
      .lean();
    const staffWithRole = await User.findById(staffUser._id)
      .populate("role")
      .lean();
    const adminWithRole = await User.findById(adminUser._id)
      .populate("role")
      .lean();
  ...
  describe("2. Security Tests - XSS (Cross-Site Scripting)", () => {
    ...
    it("should prevent XSS in audit log metadata", async () => {
      ...
      const auditLogs = await AuditLog.find({ userId: businessOwner._id })
        .limit(1)
        .lean();
  ...
  describe("5. Security Tests - Permission Bypass", () => {
    it("should prevent staff from changing password", async () => {
      ...
      expect(response.body.error).toBeDefined();
      expect(["field_restricted", "forbidden"]).toContain(
