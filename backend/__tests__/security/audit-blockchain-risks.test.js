...
function buildAuditTestApp() {
  ...
  const { auditVerifyRateLimit, auditLogRateLimit } = require('../../services/audit-service/src/middleware/rateLimit')
  ...
  const mockBlockchain = {
    verifyHash(hash) {
      const knownHashes = {
        [crypto.createHash('sha256').update('known-data').digest('hex')]: { exists: true, timestamp: Date.now() },
      }
  ...
  app.get('/api/audit/verify/:auditLogId', fakeAuth, auditVerifyRateLimit(), async (req, res) => {
    return res.json({ success: true, verified: false, auditLog: { id: req.params.auditLogId } })
  })

  // ── POST /api/audit/verify-data (mirrors real route logic) ──
  app.post('/api/audit/verify-data', fakeAuth, auditVerifyRateLimit(), async (req, res) => {
    try {
      const { data } = req.body
...
describe('Audit & Blockchain Risks — Platform Technology Risk Mitigations', () => {
      ...
      for (let i = 0; i < 22; i++) {
        promises.push(
          request(app)
            ...
            .send({ operation: 'storeHash', params: [`hash_${i}`, 'test_event'] })
        )
      }
  ...
  describe('Risk 3.9 — Hash Enumeration', () => {
    ...
    it('[Attack] rapid enumeration via GET verify is rate limited', async () => {
      ...
      for (let i = 0; i < 35; i++) {
        promises.push(
          request(app)
            .get(`/api/audit/verify/6500000000000000000000${String(i).padStart(2, '0')}`)
            .set('x-test-user-id', userId)
        )
      }
  ...
  describe('Risk 3.2 — Malicious Document Upload (magic-byte validator)', () => {
    const { validateMagicBytes } = require('../../services/admin-service/src/lib/fileValidator')
    const fs = require('fs')
    const os = require('os')
    const path = require('path')

    function writeTempFile(buffer) {
      const tmpPath = path.join(os.tmpdir(), `test-magic-${Date.now()}-${Math.random().toString(36).slice(2)}`)
    ...
    it('[Normal] valid DOCX (PK zip) passes magic-byte check', async () => {
      ...
      try {
        const result = await validateMagicBytes(tmpPath, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')
        expect(result.valid).toBe(true)
      } finally {
        fs.unlinkSync(tmpPath)
      }
  ...
  describe('Risk 3.8 — Ganache RPC Exposure (config check)', () => {
    it('[Edge] production compose override removes Ganache host port', () => {
      ...
      const prodOverride = fs.readFileSync(
        require('path').join(__dirname, '..', '..', '..', 'docker-compose.prod.yml'),
        'utf-8'
      )
