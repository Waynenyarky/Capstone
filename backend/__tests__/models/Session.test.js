...
describe('Session Model', () => {
  ...
  beforeEach(async () => {
    ...
    if (!role) {
      role = await Role.create({ name: 'Business Owner', slug: 'business_owner' })
    }
  ...
  describe('Session Queries', () => {
    it('should find active sessions for user', async () => {
      ...
      const activeSessions = await Session.find({ userId: testUser._id, isActive: true })
    ...
    it('should find expired sessions', async () => {
      ...
      const expiredSessions = await Session.find({ expiresAt: { $lt: new Date() } })
