...
router.get("/history", requireJwt, async (req, res) => {
  try {
    ...
    if (
      requestedEventType &&
      containsSqlInjection(String(requestedEventType))
    ) {
      return respond.error(
        res,
        400,
        "validation_error",
        "Invalid query parameter: SQL injection attempt detected",
      );
    }
    ...
    if (requestedUserId && isAdminRole(viewerRole)) {
      ...
      return respond.error(
        ...
        403,
        "forbidden",
        "You can only view your own audit history",
      );
    }
  ...
  try {
    ...
    if (!isOwner && !isAdmin) {
      return respond.error(
        ...
        403,
        "forbidden",
        "You do not have permission to verify this audit log",
      );
    }
  ...
  async (req, res) => {
    try {
      ...
      const performerMap = new Map(
        performerUsers.map((u) => [String(u._id), u]),
      );
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const staffUsers = await User.find({ role: { $in: roleIds } })
        .select("_id firstName lastName email office role")
        .populate("role")
        .lean();
      ...
      return respond.error(
        ...
        500,
        "recent_audit_failed",
        "Failed to retrieve recent audit activity",
      );
    }
  },
);
  ...
  try {
    ...
    if (search) {
      ...
      if (matchingUsers.length > 0) {
        ...
        query.$or = [{ eventType: searchRegex }];
      }
    }
