...
const signupPayloadSchema = Joi.object({
  ...
  email: Joi.string()
    .trim()
    .email()
    .custom(emailWithTld, "require domain TLD")
    .required(),
  ...
  termsAccepted: Joi.boolean()
    ...
    .required(),
  role: Joi.string()
    .valid(BUSINESS_OWNER_ROLE_SLUG)
    .default(BUSINESS_OWNER_ROLE_SLUG),
...
const signupStartLimiter = perEmailRateLimit({
  ...
  message:
    "You have requested multiple verification codes recently. Please wait and try again later.",
});
...
async function checkExistingEmailBeforeLimiter(req, res, next) {
  try {
    ...
    return respond.error(
      ...
      500,
      "signup_start_failed",
      `Failed to start sign up: ${err.message}`,
    );
  }
}
...
function extractPisFields(body) {
  ...
  if (body.highestEducationalAttainment)
    pis.highestEducationalAttainment = body.highestEducationalAttainment;
  // Mark PIS as completed if all required fields are present
  const hasPis = !!(
    pis.address?.street &&
    pis.address?.barangay &&
    pis.address?.city &&
    pis.address?.province &&
    pis.address?.zipCode &&
    pis.maritalStatus &&
    pis.dateOfBirth &&
    pis.placeOfBirth &&
...
router.post(
  ...
  validatePasswordStrengthMiddleware,
  validateBody(signupPayloadSchema),
  checkExistingEmailBeforeLimiter,
  signupStartLimiter,
  async (req, res) => {
    try {
      ...
      if (!emailResult || !emailResult.success) {
        console.error(
          `[Signup] Failed to send OTP email to ${email}:`,
        ...
        return respond.error(
          ...
          500,
          "email_send_failed",
          `Failed to send verification email: ${emailResult?.error || "Please check your email configuration"}`,
        );
      }
    ...
    try {
      ...
      if (!reqObj) {
        return respond.error(
          ...
          "Request expired, please sign up again",
        );
      }
      ...
      if (!emailResult || !emailResult.success) {
        console.error(
          `[Signup Resend] Failed to send OTP email to ${email}:`,
          emailResult?.error || "Unknown error",
        );
        return respond.error(
          res,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const normalizedEmail = String(p.email || emailKey)
        .toLowerCase()
        .trim();
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const expiresAt = useDB
        ? new Date(reqObj.expiresAt).getTime()
        : reqObj.expiresAt;
