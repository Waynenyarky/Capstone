...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (roleSlug !== 'admin') return respond.error(res, 403, 'forbidden', 'Step-up is only for admins')

      if (!doc.mfaSecret) return respond.error(res, 400, 'mfa_not_setup', 'MFA not set up. Use authenticator or passkey.')
...
router.post('/admin/step-up/start', requireJwt, requireRole(['admin']), async (req, res) => {
  try {
    const userId = req._userId
    ...
    if (!hasPasskeys) return respond.error(res, 400, 'no_passkeys', 'No passkeys registered. Use authenticator code for step-up.')
    ...
    return respond.error(res, 500, 'step_up_start_failed', 'Failed to start step-up')
  }
})
