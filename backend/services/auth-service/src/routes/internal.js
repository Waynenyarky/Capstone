...
router.get('/users/:id', requireInternalKey, async (req, res) => {
  try {
    const user = await User.findById(req.params.id).select('-passwordHash').populate('role').lean()
...
router.get('/users', requireInternalKey, async (req, res) => {
  try {
    ...
    const users = await User.find(filter).select('-passwordHash').populate('role').lean()
