...
const updateOwnerSchema = Joi.object({
  ...
  maritalStatus: Joi.string().valid('single', 'married', 'widowed', 'divorced', 'separated', 'legally_separated', 'annulled', 'void').allow('', null).optional(),
  placeOfBirth: Joi.string().trim().max(200).allow('', null).custom(sanitizeString).optional(),
  nationality: Joi.string().trim().max(50).allow('', null).custom(sanitizeString).optional(),
  fatherName: Joi.string().trim().max(100).allow('', null).custom(sanitizeString).optional(),
  motherName: Joi.string().trim().max(100).allow('', null).custom(sanitizeString).optional(),
  distinctiveMark: Joi.string().trim().max(200).allow('', null).custom(sanitizeString).optional(),
  highestEducationalAttainment: Joi.string().valid('elementary', 'high_school', 'vocational', 'college', 'college_undergraduate', 'college_graduate', 'postgraduate', 'others').allow('', null).optional(),
  // Reason for audit
  ...
  async (req, res) => {
    try {
      ...
      if (!isBusinessOwnerRole(roleSlug)) {
        return respond.error(res, 400, 'not_business_owner', 'User is not a business owner')
      }
      ...
      return respond.error(res, 500, 'user_fetch_failed', 'Failed to fetch user')
    }
  }
)
  ...
  async (req, res) => {
    try {
      ...
      if (body.firstName !== undefined) {
        ...
        if (sanitized === null) {
          return respond.error(res, 400, 'validation_error', 'Invalid input detected')
        }
      ...
      if (body.address !== undefined && typeof body.address === 'object') {
        const newAddress = {
          street: (body.address.street ?? user.address?.street ?? '').toString().trim(),
          barangay: (body.address.barangay ?? user.address?.barangay ?? '').toString().trim(),
          city: (body.address.city ?? user.address?.city ?? '').toString().trim(),
          province: (body.address.province ?? user.address?.province ?? '').toString().trim(),
          zipCode: (body.address.zipCode ?? user.address?.zipCode ?? '').toString().trim(),
        }
        const oldAddress = user.address || {}
        const addressChanged = ['street', 'barangay', 'city', 'province', 'zipCode'].some(
          (k) => newAddress[k] !== (oldAddress[k] || '')
        )
        if (addressChanged) {
      ...
      if (body.highestEducationalAttainment !== undefined) {
        ...
        if (val !== (user.highestEducationalAttainment || '')) {
          oldValues.highestEducationalAttainment = user.highestEducationalAttainment
      ...
      if (changes.length === 0) {
        return res.json({ success: true, updated: false, message: 'No changes detected' })
      }
      ...
      return respond.error(res, 500, 'user_update_failed', 'Failed to update user')
    }
  }
)
