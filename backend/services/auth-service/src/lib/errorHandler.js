...
function getUserFriendlyMessage(code, defaultMessage, context = {}) {
  const messages = {
    ...
    verification_required:
      "Verification is required before making this change. Please request a verification code.",
    ...
    // Password errors
    weak_password:
      "Password does not meet security requirements. Please ensure it has at least 8 characters, including uppercase, lowercase, number, and special character.",
    password_reused:
    ...
    grace_period_expired:
      "The grace period for reverting this email change has expired.",
    no_pending_change: "No pending email change request found.",

    // Field permission errors
    field_restricted:
      "This field cannot be edited by your role. This action has been logged.",
  ...
  if (context.expiresAt) {
    const remainingHours = Math.ceil(
      (new Date(context.expiresAt).getTime() - Date.now()) / (60 * 60 * 1000),
    );
