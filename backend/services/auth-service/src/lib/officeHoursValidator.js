  ...
  try {
    ...
    if (!officeHours) {
      ...
      return {
        ...
        reason: !officeHours
          ? "No office hours configured, using default business hours"
          : undefined,
      };
    }
    ...
    if (exception) {
      if (!exception.isWorkingDay) {
        return {
          isWithinHours: false,
          reason: `Holiday/exception: ${exception.reason || "Non-working day"}`,
        };
      }
      // Check if within exception hours
      if (exception.start && exception.end) {
        const isWithin = isTimeWithinRange(
          checkTime,
          exception.start,
          exception.end,
        );
        return {
          ...
          reason: isWithin
            ? undefined
            : `Outside exception hours (${exception.start}-${exception.end})`,
        };
      }
    }
    ...
    if (!dayConfig || !dayConfig.isWorkingDay) {
      return {
        isWithinHours: false,
        reason: `${dayNames[dayOfWeek]} is not a working day`,
      };
    }
    ...
    return {
      ...
      reason: isWithin
        ? undefined
        : `Outside working hours (${dayConfig.start}-${dayConfig.end})`,
    };
  } catch (error) {
    ...
    return {
      isWithinHours: true,
      reason: "Error checking office hours, defaulting to allowed",
    };
  }
}
