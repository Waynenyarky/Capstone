...
async function listAllFiles() {
  ...
  if (usersWithAvatars.length > 0) {
    ...
    usersWithAvatars.forEach((user) => {
      fileCount++;
      log(
        `   ${fileCount}. ${user.email || `${user.firstName} ${user.lastName}`}`,
        "cyan",
      );
  ...
  if (idVerifications.length > 0) {
    ...
    idVerifications.forEach((doc) => {
      if (doc.frontImageIpfsCid) {
        fileCount++;
        log(
          `   ${fileCount}. ${doc.userId?.email || "Unknown"} - Front ID`,
          "cyan",
        );
      ...
      if (doc.backImageIpfsCid) {
        fileCount++;
        log(
          `   ${fileCount}. ${doc.userId?.email || "Unknown"} - Back ID`,
          "cyan",
        );
  ...
  if (fileCount === 0) {
    ...
    log(
      "   Files are stored when users upload avatars or documents.",
      "yellow",
    );
  } else {
  ...
  try {
    ...
    pins.forEach((pin, index) => {
      const cid =
        typeof pin === "string"
          ? pin
          : pin.cid?.toString() || pin.path || "Unknown";
...
async function main() {
  ...
  const cid =
    args.find((arg) => arg.startsWith("--cid="))?.split("=")[1] ||
  ...
  try {
    ...
    // List all files
      ...
      if (ipfsAvailable) {
        ...
        log(
          "   Use --user=email@example.com to see files for a specific user",
          "yellow",
        );
        log(
          "   Use --cid=QmXxxx... to get info about a specific CID",
          "yellow",
        );
      }
    }
