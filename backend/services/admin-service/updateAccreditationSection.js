...
const { unifiedBusinessPermitSections } = require('./src/migrations/seedFormDefinitions');
...
async function updateAccreditationSection() {
  try {
    ...
    console.log('Found permit form:', permitForm.name, 'version:', permitForm.version);
    ...
    // Update the section
    permitForm.sections[accreditationSectionIndex] = updatedAccreditationSection;
    ...
    groupFields.forEach(field => {
      console.log(`- ${field.label}: ${field.type}${field.dropdownOptions ? ` with ${field.dropdownOptions.length} options` : ''}`);
    });
