...
async function backfill() {
  ...
  try {
    ...
    const groupKey = (formType, industryScope) =>
      `${formType}:${industryScope}`;
    ...
    for (const def of definitions) {
      ...
      const industryScope = effectiveIndustryScope(
        def.formType,
        def.industryScope,
        def.businessTypes,
      );
      ...
      if (!group) {
        let existing = await FormGroup.findOne({
      ...
      console.log(
        `  Updated FormDefinition ${def._id} -> formGroupId ${group._id}`,
      );
    }
