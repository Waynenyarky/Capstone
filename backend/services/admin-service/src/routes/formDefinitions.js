...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      return res.json({
        success: true,
        stats: { activated, deactivated, retired },
      });
    } catch (err) {
    ...
    try {
      ...
      if (existing) {
        return respond.error(
          ...
          400,
          "form_group_exists",
          "A form group with this type and industry already exists",
        );
      }
    ...
    try {
      ...
      if (group.retiredAt) {
        return respond.error(
          res,
          400,
          "cannot_deactivate_retired",
          "Cannot deactivate a retired form group",
        );
      }
      ...
      if (until <= new Date()) {
        return respond.error(
          ...
          400,
          "invalid_date",
          "Deactivated until must be in the future",
        );
      }
    ...
    try {
      ...
      if (definition.status !== "draft") {
        return respond.error(
          ...
          400,
          "form_not_draft",
          "Only draft versions can be set as active",
        );
      }
    ...
    try {
      ...
      if (!definition.canEdit()) {
        return respond.error(
          ...
          400,
          "form_not_editable",
          "Only draft form definitions can be edited",
        );
      }
    ...
    try {
      ...
      if (definition.status !== "pending_approval") {
        return respond.error(
          ...
          400,
          "not_pending",
          "Form definition is not pending approval",
        );
      }
    ...
    try {
      ...
      if (!definition.canArchive()) {
        return respond.error(
          ...
          400,
          "form_cannot_archive",
          "This form definition cannot be archived",
        );
      }
    ...
    try {
      ...
      if (!definition.canEdit()) {
        return respond.error(
          ...
          400,
          "form_not_editable",
          "Only draft form definitions can have files uploaded",
        );
      }
      ...
      definition.addChangeLog("updated", req._userId, {
    ...
    try {
      ...
      if (!definition.canEdit()) {
        return respond.error(
          ...
          400,
          "form_not_editable",
          "Only draft form definitions can be deleted",
        );
      }
      const userId = req._userId;
      const role = req._userRole || "admin";
    ...
    try {
      ...
      if (!definition.canEdit()) {
        return respond.error(
          ...
          400,
          "form_not_editable",
          "Only draft form definitions can have downloads removed",
        );
      }
