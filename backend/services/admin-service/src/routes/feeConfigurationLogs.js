...
router.get('/', requireJwt, requireRole(['admin']), async (req, res) => {
  try {
    ...
    const userIds = [...new Set(rawLogs.map((l) => String(l.userId)).filter(Boolean))]
    const users = userIds.length
      ? await User.find({ _id: { $in: userIds } }).select('_id email firstName lastName').lean()
      : []
    ...
    const logs = rawLogs.map((log) => {
      ...
      if (log.oldValue || log.newValue) {
        try {
          changes = {}
          if (log.oldValue) changes.old = typeof log.oldValue === 'string' && (log.oldValue.startsWith('{') || log.oldValue.startsWith('[')) ? JSON.parse(log.oldValue) : log.oldValue
          if (log.newValue) changes.new = typeof log.newValue === 'string' && (log.newValue.startsWith('{') || log.newValue.startsWith('[')) ? JSON.parse(log.newValue) : log.newValue
        } catch {
      ...
      return {
        ...
        at: log.createdAt,
        changes: changes || (log.metadata && (log.metadata.changedCategories || log.metadata.configId)) ? { ...log.metadata } : undefined,
      }
    })
    ...
    return res.status(500).json({ error: { code: 'INTERNAL', message: 'Failed to fetch fee configuration logs' } })
  }
})
