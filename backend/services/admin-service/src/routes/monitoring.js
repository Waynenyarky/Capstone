...
const SERVICE_URLS = [
  ...
  },
  {
    ...
    url: process.env.BUSINESS_SERVICE_URL || "http://localhost:3002",
  },
  {
    key: "admin",
    name: "Admin Service",
    url: process.env.ADMIN_SERVICE_URL || "http://localhost:3003",
  },
  {
    key: "audit",
    name: "Audit Service",
...
async function checkServiceHealth({ key, name, url }) {
  ...
  try {
    ...
    const ok =
      res.status === 200 &&
      res.data &&
      (res.data.ok === true || res.data.ok === undefined);
    return {
      key,
      name,
...
async function checkAiServiceHealth() {
  const aiUrl =
    ...
    process.env.LOB_MODEL_SERVICE_URL ||
    "http://localhost:5050";
  try {
...
async function getIpfsStatus() {
  try {
    ...
    const isAvailable =
      typeof ipfsService.isAvailable === "function" &&
  ...
  async (req, res) => {
    try {
      ...
      if (adminIndex >= 0) {
        ...
        results[adminIndex].status =
          mongoose.connection.readyState === 1 ? "up" : "degraded";
        results[adminIndex].database =
      ...
      logger.error("Services health aggregation failed", {
        error: err,
...
router.get("/stats", requireJwt, requireRole(["admin"]), async (req, res) => {
  try {
    ...
    return respond.error(
      res,
      500,
      "monitoring_stats_failed",
      "Failed to retrieve monitoring statistics",
    );
  }
});
  ...
  async (req, res) => {
    try {
      ...
      const announcementId = req.query.announcementId
        ? String(req.query.announcementId).trim()
        : null;
      ...
      if (resourceType) {
        ...
        query.$or = [{ eventType: regex }, { fieldChanged: regex }];
      }
      if (dateFrom || dateTo) {
        query.createdAt = {};
