  ...
  try {
    ...
    if (!type) {
      return res.status(400).json({
        ...
        error: "type_required",
        message:
          "Form type is required (permit, renewal, cessation, violation, or appeal)",
      });
    }
    ...
    const definition = await FormDefinition.findActiveDefinition(
      type,
      businessType,
      lgu,
    );
      ...
      if (group?.deactivatedUntil) {
        ...
        if (new Date(group.deactivatedUntil) > now) {
          return res.json({
            ...
            reason:
              group.deactivateReason || "This form is temporarily unavailable.",
          });
        }
      }
    }
