...
router.post("/incidents", requireInternalApiKey, async (req, res) => {
  try {
    ...
    if (eventType === "staff_or_admin_forgot_password_attempted") {
      ...
      if (!userId || !userEmail) {
        return respond.error(
          ...
          400,
          "validation_error",
          "userId and userEmail are required",
        );
      }
      ...
      const now = new Date();
      const affectedUserId = mongoose.Types.ObjectId.isValid(userId)
      ...
      logger.info(
        "Security incident created (staff/admin forgot-password attempt)",
        {
    ...
    if (eventType === "audit_tamper_detected") {
      ...
      if (!auditLogId || !userId) {
        return respond.error(
          ...
          400,
          "validation_error",
          "auditLogId and userId are required for audit_tamper_detected",
        );
      }
      ...
      const status = validStatuses.includes(verificationStatus)
        ? verificationStatus
        : "verification_error";
      const sev =
        severity === "high" || severity === "medium"
          ...
          : status === "tamper_detected"
            ...
            : "medium";
      const now = new Date();
      const auditLogObjId = mongoose.Types.ObjectId.isValid(auditLogId)
        ? new mongoose.Types.ObjectId(auditLogId)
        : null;
      const affectedUserId = mongoose.Types.ObjectId.isValid(userId)
        ? new mongoose.Types.ObjectId(userId)
        : null;
      ...
      if (!incident) {
        ...
        if (
          affectedUserId &&
          !incident.affectedUserIds.map(String).includes(String(affectedUserId))
        ) {
          incident.affectedUserIds.push(affectedUserId);
        }
        if (
          auditLogObjId &&
          !incident.auditLogIds.map(String).includes(String(auditLogObjId))
        ) {
    ...
    return respond.error(
      ...
      "Unsupported event type for incident creation",
    );
  } catch (error) {
    ...
    try {
      ...
      if (process.env.NODE_ENV === "test") {
        try {
          ...
          const count =
            await TamperIncident.countDocuments(query).maxTimeMS(5000);
          if (count > 0) {
          ...
          logger.warn("Count query failed, assuming empty collection", {
            error: countError,
          });
      ...
      return respond.error(
        res,
        500,
        "tamper_incidents_list_failed",
        "Failed to retrieve tamper incidents",
      );
    }
  },
);
  ...
  async (_req, res) => {
    try {
      ...
      if (process.env.NODE_ENV === "test") {
        try {
          [total, open, acknowledged, resolved] = await Promise.all([
            ...
            TamperIncident.countDocuments({ status: "acknowledged" }).maxTimeMS(
              5000,
            ),
            TamperIncident.countDocuments({ status: "resolved" }).maxTimeMS(
              5000,
            ),
          ]);
        } catch (countError) {
          logger.warn("Count queries failed for stats, returning zeros", {
            error: countError,
          });
      ...
      return respond.error(
        res,
        500,
        "tamper_incidents_stats_failed",
        "Failed to retrieve incident stats",
      );
    }
  },
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const incident = await TamperIncident.findByIdAndUpdate(id, update, {
        new: true,
      });
  ...
  async (req, res) => {
    try {
      ...
      return respond.error(
        res,
        500,
        "tamper_incident_contain_failed",
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const { resolutionNotes = "", containmentActive = false } =
        req.body || {};
      ...
      return respond.error(
        res,
        500,
        "tamper_incident_resolve_failed",
...
router.get("/incidents/contained-users", async (req, res) => {
  try {
    const incidents = await TamperIncident.find({
      ...
      .select("affectedUserIds")
      .lean();

    const userIds = [
      ...new Set(
        incidents.flatMap((i) => (i.affectedUserIds || []).map(String)),
      ),
    ];
