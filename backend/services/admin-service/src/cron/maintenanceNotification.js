...
async function checkAndSendMaintenanceNotifications() {
  try {
    ...
    logger.info(
      `Found ${upcomingMaintenance.length} upcoming maintenance windows`,
    );

    for (const maintenance of upcomingMaintenance) {
      const scheduledStartAt = new Date(
        maintenance.requestDetails.scheduledStartAt,
      );
      ...
      if (notificationSent) {
        logger.info(
          `Notification already sent for maintenance ${maintenance.approvalId}`,
        );
        continue;
      }
      ...
      for (const user of users) {
        await createInAppNotification(
          ...
          logger.warn(`Failed to send notification to user ${user._id}`, {
            err,
          }),
        );
      }
      ...
      await AdminApproval.updateOne(
        { approvalId: maintenance.approvalId },
        {
          ...
          "metadata.notificationSentAt": new Date(),
        },
      ).catch((err) =>
        logger.warn("Failed to mark notification as sent", { err }),
      );

      logger.info(
        `Successfully sent notifications for maintenance ${maintenance.approvalId}`,
      );
    }
