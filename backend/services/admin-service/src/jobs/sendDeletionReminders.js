...
async function sendDeletionReminders() {
  try {
    ...
    for (const user of usersWithPendingDeletion) {
      try {
        ...
        if (daysRemaining === 7 || daysRemaining === 3 || daysRemaining === 1) {
          ...
          logger.info(
            `Deletion reminder for ${user.email}: ${daysRemaining} days remaining`,
            {
        ...
        logger.error(`Error sending deletion reminder to ${user.email}`, {
          error,
          userId: String(user._id),
        });
    ...
    logger.info(`Sent ${sentCount} deletion reminder(s)`, {
      sentCount,
