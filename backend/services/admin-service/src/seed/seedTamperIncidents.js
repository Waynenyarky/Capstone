...
async function seedTamperIncidentsIfEmpty() {
  ...
  if (!enabled) {
    return {
      seeded: false,
      reason: "SEED_TAMPER_INCIDENTS or SEED_DEV not set",
    };
  }
  ...
  try {
    ...
    if (existing > 0) {
      logger.info(
        "Tamper incidents seed: collection already has documents, skipping.",
      );
      return {
        seeded: false,
        reason: "already has incidents",
        count: existing,
      };
    }
    ...
    await TamperIncident.create({
      ...
      message:
        "Seed: Hash mismatch for audit log entry; on-chain verification failed.",
    ...
    await TamperIncident.create({
      ...
      verificationStatus: "tamper_detected",
      message:
        "Seed: Hash mismatch acknowledged; containment active for affected accounts.",
      affectedUserIds: adminId ? [adminId] : [],
      auditLogIds: [new mongoose.Types.ObjectId()],
    ...
    await TamperIncident.create({
      ...
      verificationStatus: "tamper_detected",
      message:
        "Seed: Audit log hash mismatch; verified against blockchain and documented.",
      affectedUserIds: [],
      auditLogIds: [new mongoose.Types.ObjectId()],
      containmentActive: false,
      resolutionNotes:
        "Confirmed on-chain hash matches expected value after re-verification. No data change; incident logged for audit.",
    ...
    await TamperIncident.create({
      ...
      containmentActive: false,
      resolutionNotes:
        "Chain was temporarily unavailable. Re-ran verification after recovery; all logs verified.",
    ...
    await TamperIncident.create({
      ...
      containmentActive: false,
      resolutionNotes:
        "Log was still in queue. Next run confirmed it was logged and verified.",
