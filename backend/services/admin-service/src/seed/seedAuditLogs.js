...
async function seedAuditLogsIfEmpty() {
  ...
  try {
    const existingSeed = await AuditLog.findOne({ metadata: SEED_MARKER }).limit(1).lean();
    if (existingSeed) {
    ...
    if (!admin) {
      logger.info('Audit logs seed: admin@example.com not found. Run auth SEED_DEV first.');
      return { seeded: false, reason: 'missing admin user' };
    }

    const admin2 = await User.findOne({ email: 'admin2@example.com', isActive: true }).select('_id').lean();
    const userId = admin._id;
    const userId2 = admin2 ? admin2._id : userId;

    const entries = [
      { eventType: 'login', role: 'admin', createdAt: hoursAgo(1) },
      { eventType: 'admin_approval_approved', role: 'admin', createdAt: hoursAgo(3) },
      { eventType: 'admin_approval_request', role: 'admin', createdAt: hoursAgo(8) },
      { eventType: 'login', role: 'admin', createdAt: hoursAgo(12) },
      { eventType: 'admin_approval_rejected', role: 'admin', createdAt: hoursAgo(20) },
      { eventType: 'profile_update', role: 'admin', fieldChanged: 'phoneNumber', createdAt: hoursAgo(28) },
      { eventType: 'login', role: 'admin', createdAt: hoursAgo(36) },
      { eventType: 'admin_approval_approved', role: 'admin', createdAt: hoursAgo(42) },
    ];
