...
async function seedApprovalRequestsIfEmpty() {
  ...
  if (!enabled) {
    return {
      seeded: false,
      reason: "SEED_APPROVAL_REQUESTS or SEED_DEV not set",
    };
  }
  ...
  try {
    const admins = await User.find({
      email: {
        $in: ["admin@example.com", "admin2@example.com", "admin3@example.com"],
      },
    })
      .select("_id email firstName lastName")
      .lean();
    const staff = await User.find({
      email: { $in: ["officer@example.com", "inspector@example.com"] },
    })
      .select("_id email firstName lastName")
      .lean();
    ...
    if (admins.length < 2 || staff.length < 1) {
      logger.info(
        "Seed approval requests: need at least 2 admins and 1 staff (e.g. officer@example.com). Run auth SEED_DEV first.",
      );
        ...
        approvals: [
          {
            ...
            approved: true,
            comment: "",
            timestamp: approvedAt3,
          },
          {
            adminId: admin3._id,
            approved: true,
            comment: "OK for window.",
            timestamp: approvedAt3,
          },
        ],
        ...
        approvals: [
          {
            adminId: admin1._id,
            approved: false,
            comment: "Postpone to weekend.",
            timestamp: rejectedMaintAt,
          },
          {
            adminId: admin3._id,
            approved: false,
        ...
        approvals: [
          {
            adminId: admin1._id,
            approved: false,
            comment: "Ask user to use forgot password.",
            timestamp: rejectedAt3,
          },
          {
            adminId: admin2._id,
            approved: false,
