...
async function seedAnnouncementsIfEmpty() {
  ...
  try {
    ...
    const adminRole = await Role.findOne({ slug: 'admin' }).select('_id').lean();
    if (!adminRole) {
      logger.info('Announcement seed: admin role not found. Run auth SEED_DEV first.');
      return { seeded: false, reason: 'missing admin role' };
    }

    const admin = await User.findOne({ role: adminRole._id, isActive: true }).select('_id').lean();
    if (!admin) {
      logger.info('Announcement seed: no active admin user found. Run auth SEED_DEV first.');
...
For questions, contact the Zoning Compliance Office at (555) 234-5678.`,
      ...
      {
        title: 'Health & Safety Inspection Period - Schedule Your Appointment Now',
        audience: 'public',
...
We will resume normal business hours at 8 AM on the day following each closure. We appreciate your patience and wish you a happy holiday season!`,
      ...
      {
        title: 'New Environmental Compliance Standards - Effective April 1, 2026',
        audience: 'public',
...
We recommend reviewing these standards immediately and beginning implementation to ensure smooth compliance before the enforcement date.`,
      ...
      {
        title: 'Updated ADA Accessibility Requirements - Immediate Implementation',
        audience: 'public',
