  ...
  if (!stepUpToken) {
    return res.status(403).json({
      error: {
        ...
        message: 'This action requires re-authentication. Please complete step-up (authenticator or passkey) and retry.',
      },
    })
  }
  try {
    ...
    if (!decoded || decoded.stepUp !== true) {
      return res.status(403).json({
        error: { code: 'invalid_step_up', message: 'Invalid or expired step-up. Please complete step-up again.' },
      })
    }
    if (String(decoded.sub) !== String(req._userId)) {
      return res.status(403).json({
        error: { code: 'step_up_user_mismatch', message: 'Step-up token does not match current user.' },
      })
    }
    next()
  } catch (err) {
