...
FormDefinitionSchema.statics.findActiveDefinition = async function (
  ...
  const scored = definitions.map((def) => {
    ...
    const matchesBusinessType =
      hasBusinessTypes &&
      businessType &&
      def.businessTypes.includes(businessType);

    // LGU matching
    const hasLguCodes = def.lguCodes && def.lguCodes.length > 0;
    const matchesLgu =
      hasLguCodes && lguCode && def.lguCodes.includes(lguCode.toUpperCase());
...
FormDefinitionSchema.methods.addChangeLog = function (
  action,
  userId,
  changes = {},
  comment = "",
