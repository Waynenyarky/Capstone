...
const IPFS_GATEWAY = (process.env.IPFS_GATEWAY_URL || 'http://localhost:8080/ipfs/').replace(/\/?$/, '/')
...
async function listApplicationFiles(filterAppRef = null) {
  ...
  const query = {}
  if (filterAppRef) {
    query['businesses.applicationReferenceNumber'] = new RegExp(filterAppRef, 'i')
  }
  ...
  for (const profile of profiles) {
    ...
    for (const biz of profile.businesses || []) {
      ...
      if (profile.ownerIdentity?.idFileIpfsCid) {
        entries.push({ label: 'Owner ID file', cid: profile.ownerIdentity.idFileIpfsCid })
      }
      ...
      for (const [k, v] of Object.entries(lgu)) {
        if (typeof v === 'string' && v.trim() && (k === 'ipfsCid' || k.endsWith('IpfsCid'))) {
      ...
      ;['certificateIpfsCid', 'booksOfAccountsIpfsCid', 'authorityToPrintIpfsCid', 'paymentReceiptIpfsCid'].forEach((f) => {
        if (bir[f]) entries.push({ label: f, cid: bir[f].trim() })
      })

      // Other agencies
      const oa = biz.otherAgencyRegistrations || {}
      if (oa.sss?.proofIpfsCid) entries.push({ label: 'SSS proof', cid: oa.sss.proofIpfsCid.trim() })
      if (oa.philhealth?.proofIpfsCid) entries.push({ label: 'PhilHealth proof', cid: oa.philhealth.proofIpfsCid.trim() })
      if (oa.pagibig?.proofIpfsCid) entries.push({ label: 'Pag-IBIG proof', cid: oa.pagibig.proofIpfsCid.trim() })
      ...
      for (const r of biz.renewals || []) {
        ...
        for (const [k, v] of Object.entries(rd)) {
          if (typeof v === 'string' && v.trim() && k.endsWith('IpfsCid')) {
            entries.push({ label: `Renewal ${r.renewalYear}: ${k}`, cid: v.trim() })
          }
        }
      }
  ...
  if (total === 0) {
    ...
    log('   Submit a new application with file uploads (or use prefill test data) to see CIDs here.', 'yellow')
  } else {
    log(`\n✅ Total: ${total} document(s). Open URLs in your browser to view files.`, 'green')
  }
}
...
async function listPinned() {
  ...
  try {
    ...
    log('   Start IPFS (e.g. docker-compose up -d ipfs) or use list from DB only.', 'yellow')
  }
}

async function main() {
  const args = process.argv.slice(2)
  const filterApp = args.find((a) => a.startsWith('--application='))?.split('=')[1] || null
