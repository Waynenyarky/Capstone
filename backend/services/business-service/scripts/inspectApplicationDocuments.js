    ...
    for (const biz of profile.businesses || []) {
      ...
      if (lguKeys.length === 0) {
        ...
        for (const k of lguKeys) {
          ...
          console.log(
            `   ${k}: ${hasValue ? `"${v.substring(0, 20)}..." (${v.length} chars)` : "(empty)"}`,
          );
        }
      }
      ...
      console.log("");
      console.log(
        "formData keys that look like file fields (array with cid, or string):",
      );
      const formData =
        biz.formData && typeof biz.formData === "object" ? biz.formData : {};
      const fileLikeKeys = Object.keys(formData).filter((key) => {
        ...
        if (Array.isArray(v) && v.length > 0) {
          ...
          return (
            ...
            typeof first === "object" &&
            (first.cid != null ||
              (first.response && first.response.cid != null))
          );
        }
        return false;
      });
      if (fileLikeKeys.length === 0) {
        ...
        fileLikeKeys.forEach((key) => {
          ...
          console.log(
            `   ${key}: ${cid ? `cid="${cid.substring(0, 24)}..."` : "(no cid)"}`,
          );
        });
      }
      ...
      console.log("");
      console.log(
        "Documents object as returned by LGU review API (fixed + dynamic from lguDocuments):",
      );
      const fixedKeys = [
        "idPicture",
        "ctc",
