...
function getDatasetPath() {
  if (
    process.env.LOB_DATASET_PATH &&
    fs.existsSync(process.env.LOB_DATASET_PATH)
  ) {
    return process.env.LOB_DATASET_PATH;
  }
  const fromSeed = (levelsUp, ...rest) =>
    path.resolve(__dirname, "..", ...Array(levelsUp).fill(".."), ...rest);
  const candidates = [
    ...
    fromSeed(
      ...
      "ai",
      "datasets",
      "lob_recommendation_dataset_bootstrapped.json",
    ), // Fallback: extended set
    fromSeed(
      5,
      "ai",
      "datasets",
...
async function seedIfEmpty() {
  ...
  if (!DATASET_PATH) {
    return {
      seeded: false,
      count: 0,
      skipped: "dataset_not_found",
      triedPaths: getDatasetPathForLog(),
    };
  }
