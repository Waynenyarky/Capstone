...
try {
  ...
  logger.warn(
    "[renewalAutoFlag] PenaltyConfiguration model not available — using defaults",
  );
}
...
async function flagBusinessesForRenewal() {
  ...
  logger.info(
    `[Renewal Cron] Flagging businesses for renewal year ${currentYear}`,
  );
  ...
  try {
    ...
    logger.info(
      `[Renewal Cron] Flagged ${flagged} businesses for renewal year ${currentYear}`,
    );
    return { flagged, year: currentYear };
  } catch (err) {
    logger.error("[Renewal Cron] Error flagging businesses:", {
      error: err.message,
    });
    throw err;
  }
...
async function calculateMonthlyInterest() {
  ...
  logger.info(
    `[Interest Cron] Calculating monthly interest for ${currentYear}-${currentMonth}`,
  );
  ...
  try {
    ...
    try {
      if (PenaltyConfiguration) {
        penaltyConfig = await PenaltyConfiguration.findOne({
          isActive: true,
        }).lean();
      }
    } catch (_) {
      ...
      for (const business of profile.businesses) {
        for (const renewal of business.renewals || []) {
          ...
          if (monthsOverdue > 0 && renewal.assessment) {
            ...
            const interest =
              (baseFees + surcharge) * (monthlyRate / 100) * monthsOverdue;

            renewal.assessment.surcharge = Math.round(surcharge * 100) / 100;
            renewal.assessment.interest = Math.round(interest * 100) / 100;
            renewal.assessment.totalWithPenalty =
              Math.round((baseFees + surcharge + interest) * 100) / 100;
    ...
    logger.info(
      `[Interest Cron] Updated ${updated} overdue renewals with interest`,
    );
    return { updated, monthlyRate, surchargeRate };
  } catch (err) {
    logger.error("[Interest Cron] Error calculating interest:", {
      error: err.message,
    });
    throw err;
  }
