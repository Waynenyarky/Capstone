...
async function detectAbandonedBusinesses() {
  ...
  logger.info(`[Abandoned Cron] Checking for abandoned businesses (year: ${currentYear})`)
  ...
  try {
    ...
    for (const profile of profiles) {
      for (const business of profile.businesses) {
        ...
        if (business.retirementStatus && ['requested', 'confirmed'].includes(business.retirementStatus)) continue
    ...
    logger.info(`[Abandoned Cron] Found ${flagged} potentially abandoned businesses`)
    return { flagged, businesses: abandonedBusinesses }
  } catch (err) {
    logger.error('[Abandoned Cron] Error detecting abandoned businesses:', { error: err.message })
    throw err
  }
}
...
async function markBusinessAbandoned(businessId, inspectorId) {
  try {
    ...
    await profile.save()
    logger.info(`[Abandoned] Business ${businessId} marked as inactive by inspector ${inspectorId}`)
    return { businessId, status: 'inactive' }
  } catch (err) {
    logger.error('[Abandoned] Error marking business:', { businessId, error: err.message })
    throw err
  }
}
