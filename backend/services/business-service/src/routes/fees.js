...
router.post('/assessment', requireJwt, async (req, res) => {
  try {
    ...
    return respond.error(res, 500, 'assessment_error', err.message || 'Failed to compute fee assessment')
  }
})
...
router.post('/what-if', requireJwt, async (req, res) => {
  try {
    ...
    if (!businessData.lineOfBusiness) {
      return respond.error(res, 400, 'missing_lob', 'Line of business is required')
    }
    ...
    return respond.error(res, 500, 'calculation_error', err.message || 'Fee calculation failed')
  }
})
...
router.post('/impact/:businessId', requireJwt, async (req, res) => {
  try {
    ...
    if (!changes || Object.keys(changes).length === 0) {
      return respond.error(res, 400, 'missing_changes', 'Changes are required for impact analysis')
    }

    const result = await whatIfFeeService.getFeeImpactAnalysis(businessId, changes)
    res.json(result)
  } catch (err) {
    console.error('POST /api/business/fees/impact error:', err)
    return respond.error(res, 500, 'impact_error', err.message || 'Impact analysis failed')
  }
})
...
router.post('/compare', requireJwt, async (req, res) => {
  try {
    ...
    if (!scenarios || !Array.isArray(scenarios) || scenarios.length < 2) {
      return respond.error(res, 400, 'invalid_scenarios', 'At least 2 scenarios are required for comparison')
    }
    ...
    return respond.error(res, 500, 'comparison_error', err.message || 'Scenario comparison failed')
  }
})
...
router.post('/breakdown', requireJwt, async (req, res) => {
  try {
    ...
    return respond.error(res, 500, 'breakdown_error', err.message || 'Fee breakdown failed')
  }
})
...
router.get('/history/:businessId?', requireJwt, async (req, res) => {
  try {
    ...
    const history = businessId ? [
      {
        date: '2024-01-15',
        businessId,
        fees: { baseFee: 500, sanitaryFee: 200, fireSafetyFee: 150, regulatoryFees: 100, totalFees: 950 },
        changes: 'Initial registration'
      },
      {
        date: '2024-07-15',
        businessId,
        fees: { baseFee: 750, sanitaryFee: 300, fireSafetyFee: 200, regulatoryFees: 150, totalFees: 1400 },
    ...
    return respond.error(res, 500, 'history_error', err.message || 'Failed to get fee history')
  }
})
...
router.get('/optimization/:businessId', requireJwt, async (req, res) => {
  try {
    ...
    const recommendations = whatIfFeeService.generateOptimizationRecommendations(currentData, fees)
    ...
    return respond.error(res, 500, 'optimization_error', err.message || 'Failed to get optimization suggestions')
  }
})
...
router.post('/project', requireJwt, async (req, res) => {
  try {
    ...
    if (!currentData || !growthParameters) {
      return respond.error(res, 400, 'missing_data', 'Current data and growth parameters are required')
    }
    ...
    for (let year = 1; year <= years; year++) {
      ...
      projectedData.grossAnnualSales *= (1 + (growthParameters.salesGrowth || 0.05))
      projectedData.numberOfEmployees = Math.floor(projectedData.numberOfEmployees * (1 + (growthParameters.employeeGrowth || 0.03)))
      projectedData.capitalInvestment *= (1 + (growthParameters.capitalGrowth || 0.04))
    ...
    return respond.error(res, 500, 'projection_error', err.message || 'Fee projection failed')
  }
})
...
function generatePaymentSchedule(fees) {
  return {
    ...
    installment: {
      ...
      schedule: fees.totalFees > 1000 ? [
        { dueDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], amount: fees.totalFees * 0.5 },
        { dueDate: new Date(Date.now() + 60 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], amount: fees.totalFees * 0.5 }
      ] : [],
