...
const { LINE_OF_BUSINESS } = require('../../../../shared/constants/lineOfBusiness')
...
function loadDataset() {
  ...
  const filePath = path.join(__dirname, '..', '..', '..', '..', '..', 'ai', 'datasets', 'lob_recommendation_dataset.json')
  try {
    const raw = fs.readFileSync(filePath, 'utf-8')
    dataset = JSON.parse(raw)
    logger.info('LOB recommendation dataset loaded', { entries: dataset.length })
  } catch (err) {
    logger.warn('Failed to load LOB recommendation dataset', { error: err.message, path: filePath })
    dataset = []
  }
  return dataset
}
...
for (const lob of LINE_OF_BUSINESS) {
  for (const dl of lob.detailedLines) {
    VALID_DETAILED_LINES.set(`${lob.taxCode}:${dl.toLowerCase()}`, { taxCode: lob.taxCode, lineOfBusiness: lob.lineOfBusiness, detailedLine: dl })
  }
}
...
function findClosestDetailedLine(taxCode, candidate) {
  ...
  const match = lob.detailedLines.find(dl => dl.toLowerCase().includes(lower) || lower.includes(dl.toLowerCase()))
  if (match) {
    const idx = lob.detailedLines.indexOf(match)
    return { taxCode: lob.taxCode, lineOfBusiness: lob.lineOfBusiness, detailedLine: match, psicCode: lob.psicCodes[idx] || '' }
  }
  return null
}
...
function buildPrompt(userDescription) {
  ...
  const fewShot = (Array.isArray(data) ? data.slice(0, 20) : []).map(entry => {
  ...
  const taxonomyList = LINE_OF_BUSINESS.map(lob =>
...
async function tryTrainedModel(description) {
  ...
  try {
    ...
    if (!res.ok) {
      logger.warn('Trained model service returned error', { status: res.status })
      return null
    }
    ...
    if (data.noConfidentMatch) {
      return {
        recommendations: [],
        message: data.message || 'Your description doesn\'t clearly match any of our current lines of business. Please add your line(s) manually below.',
        noConfidentMatch: true,
      }
    }
    return null
  } catch (err) {
    logger.warn('Trained model service call failed, falling back to Gemini', { error: err.message })
    return null
  }
}
...
const BUSINESS_KEYWORDS = [
  ...
  'sell', 'selling', 'operate', 'operating', 'run', 'running', 'own', 'owning',
  'provide', 'providing', 'offer', 'offering', 'supply', 'supplying',
  'distribute', 'distributing', 'manufacture', 'manufacturing', 'produce', 'producing',
  'make', 'making', 'process', 'processing', 'fabricate', 'fabricating',
  'assemble', 'assembling', 'build', 'building', 'construct', 'constructing',
  'repair', 'repairing', 'fix', 'fixing', 'install', 'installing',
  'deliver', 'delivering', 'haul', 'hauling', 'transport', 'transporting',
  'rent', 'renting', 'lease', 'leasing', 'manage', 'managing',
  'serve', 'serving', 'cater', 'catering', 'cook', 'cooking',
  'grow', 'growing', 'raise', 'raising', 'farm', 'farming',
  'quarry', 'quarrying', 'mine', 'mining', 'extract', 'extracting',
  'deploy', 'deploying', 'help', 'helping', 'assist', 'assisting',
  'develop', 'developing', 'design', 'designing', 'handle', 'handling',
  
  // Core business verbs (Filipino)
  'nagbebenta', 'nagtitinda', 'nag-aalok', 'nag-ooperate', 'nag-susuply',
  'nagluluto', 'nag-cacater', 'nag-lalaba', 'nag-paplantsa',
  'nagkukumpuni', 'nag-aayos', 'nag-iinstall', 'nag-piprint',
  'nag-poproseso', 'gumagawa', 'gumugupit', 'nag-aalaga', 'nagpapagamot',
  'nagtatanim', 'nag-aalaga', 'naghahatid', 'nagpapaupa', 'tumutulong',
  'kumukuha', 'nangongolekta', 'ginagawang', 'nagpapatayo',
  
  // Ownership & possession (English & Filipino)
  'i have', 'we have', 'i am', 'we are', 'i do', 'we do',
  'may', 'kami', 'ako', 'mayroon',
  
  // Business types & establishments (English)
  'store', 'shop', 'business', 'company', 'firm', 'agency', 'center',
  'restaurant', 'eatery', 'cafe', 'bar', 'bakery', 'canteen',
  'salon', 'barbershop', 'parlor', 'clinic', 'hospital',
  'pharmacy', 'drugstore', 'hardware', 'market', 'mall',
  'factory', 'plant', 'mill', 'workshop', 'warehouse',
  'office', 'station', 'terminal', 'depot', 'garage',
  'farm', 'ranch', 'fishpond', 'nursery', 'plantation',
  'hotel', 'inn', 'motel', 'resort', 'lodge', 'pension',
  'school', 'academy', 'institute', 'training', 'tutorial',
  'bank', 'lending', 'pawnshop', 'remittance', 'insurance',
  'contractor', 'builder', 'developer', 'broker', 'dealer',
  
  // Business types & establishments (Filipino)
  'tindahan', 'negosyo', 'kumpanya', 'ahensya', 'sentro',
  'karinderia', 'panaderia', 'botika', 'sanglaan',
  'gilingan', 'pabrika', 'bodega', 'garahe',
  'bukid', 'sakahan', 'taniman', 'palayan',
  'paaralan', 'eskwelahan', 'ospital', 'klinika',
  
  // Business activities & outputs (English)
  'retail', 'wholesale', 'service', 'services', 'sales',
  'food', 'goods', 'products', 'items', 'merchandise', 'wares',
  'materials', 'supplies', 'equipment', 'machinery', 'tools',
  'construction', 'building', 'installation', 'maintenance',
  'delivery', 'shipping', 'logistics', 'freight', 'cargo',
  'rental', 'leasing', 'property', 'real estate',
  'advertising', 'marketing', 'media', 'printing', 'publishing',
  'consulting', 'consultancy', 'advisory', 'accounting', 'bookkeeping',
  'legal', 'law', 'attorney', 'lawyer', 'notary',
  'medical', 'dental', 'health', 'veterinary', 'clinic',
  'security', 'manpower', 'recruitment', 'employment',
  'laundry', 'cleaning', 'washing', 'pressing',
  'transportation', 'trucking', 'courier', 'express',
  
  // Business activities & outputs (Filipino)
  'serbisyo', 'pagkain', 'produkto', 'gamit', 'materyales',
  'kagamitan', 'kasangkapan', 'paninda', 'tinda', 'benta',
  'karne', 'gulay', 'isda', 'manok', 'baboy', 'baka',
  'damit', 'sapatos', 'cellphone', 'laptop', 'gadget',
  'semento', 'buhangin', 'gravel', 'hollow blocks',
  'gamot', 'vitamins', 'alahas', 'prenda',
  'bigas', 'palay', 'mais', 'pataba', 'binhi',
  'ulam', 'kanin', 'tinapay', 'pandesal', 'cake',
  'groceries', 'vegetables', 'fruits', 'rice',
  
  // Professional roles (English)
  'contractor', 'supplier', 'distributor', 'manufacturer', 'producer',
  'retailer', 'wholesaler', 'vendor', 'merchant', 'trader',
  'agent', 'broker', 'dealer', 'consultant', 'specialist',
  'technician', 'mechanic', 'electrician', 'plumber', 'carpenter',
  'farmer', 'grower', 'breeder', 'fisher', 'rancher',
  'driver', 'operator', 'manager', 'owner', 'proprietor',
  'developer', 'builder', 'designer', 'engineer', 'architect',
  
  // Professional roles (Filipino)
  'magsasaka', 'mangingisda', 'karpintero', 'mekaniko',
  'drayber', 'kontraktor', 'tsuper', 'barbero',
  
  // Common business contexts
  'customers', 'clients', 'buyers', 'market', 'wholesale', 'bulk',
  'online', 'delivery', 'pickup', 'order', 'orders',
  'licensed', 'registered', 'certified', 'accredited',
]
...
const VAGUE_PHRASE_BLACKLIST = new Set([
  'my business', 'our business', 'a business', 'the business',
  'my company', 'our company', 'a company', 'the company',
  'my store', 'our store', 'a store', 'the store',
  'my shop', 'our shop', 'a shop', 'the shop',
  'my negosyo', 'our negosyo', 'negosyo',
  'my tindahan', 'our tindahan', 'tindahan',
  'i have a business', 'we have a business',
  'we run a business', 'i run a business',
  'i have a company', 'we have a company',
  'i have a store', 'we have a store',
  'i have a shop', 'we have a shop',
  // Generic-only: no specifics about what is sold or done
  'my business sells products', 'my business sells goods', 'our business sells products',
  'we sell products', 'we sell goods', 'we offer services', 'i sell products', 'i sell goods',
  'we have a business that sells products', 'i have a store that sells things',
  'we offer services to customers', 'we provide services', 'we sell things',
  'my business offers services', 'our company sells products',
  // Filipino equivalents
  'negosyo ko nagbebenta ng produkto', 'nagtitinda kami ng produkto',
  'nagbebenta kami ng paninda', 'nag-aalok kami ng serbisyo',
  'negosyo namin nagbebenta ng gamit', 'tindahan namin nagbebenta ng produkto',
])
...
function isLikelyBusinessDescription(description) {
  ...
  const matchedWords = singleWordKeywords.filter(keyword => words.includes(keyword))
...
Text: "${description}"`
    ...
    logger.warn('Gemini gate check failed, assuming valid description', { error: err.message })
    return true
  }
}
...
const FALLBACK_HELP_TIP = 'We need a bit more detail to suggest the right line of business. Try adding what you sell or do — for example: "sari-sari store selling snacks and drinks" or "restaurant serving Filipino food".'
...
Your reply:`
    const timeoutPromise = new Promise((_, reject) =>
      setTimeout(() => reject(new Error('Help tip timeout')), HELP_TIP_TIMEOUT_MS)
    )
    ...
    if (text && typeof text === 'string') {
      ...
      const exactThankOnly = ['salamat sa paglalahad!', 'thanks for sharing!', 'thank you!'].includes(trimmed.toLowerCase())
      if (!tooShort && !exactThankOnly) {
        return trimmed
      }
      logger.warn('Gemini help tip reply rejected (too short or thank-only), using fallback', {
  ...
  async (req, res) => {
    try {
      ...
      if (!businessDescription || typeof businessDescription !== 'string') {
        return respond.error(res, 400, 'invalid_input', 'businessDescription is required and must be a string')
      }

      const sanitized = sanitizeString(businessDescription)
      if (sanitized.length < 10) {
        return respond.error(res, 400, 'input_too_short', 'Please provide a more detailed description (at least 10 characters)')
      }
      if (sanitized.length > MAX_DESCRIPTION_LENGTH) {
        return respond.error(res, 400, 'input_too_long', `Description must not exceed ${MAX_DESCRIPTION_LENGTH} characters`)
      }
      ...
      const genericRetailMessage = "Your description is general. We've suggested 'General merchandise' — please confirm or add more specific lines below."
      ...
      if (!isLikelyBusinessDescription(sanitized)) {
        logger.info('Input rejected by heuristic (not likely a business description)')
        ...
        return respond.ok(res, 200, { recommendations: [], message, geminiReview: null })
      }

      // When GEMINI_API_KEY is set, also run Gemini gate for edge cases the heuristic might miss.
      if (apiKeyForGate) {
        const genAI = new GoogleGenerativeAI(apiKeyForGate)
        const isBusinessDesc = await askGeminiIsBusinessDescription(sanitized, genAI)
        if (!isBusinessDesc) {
          logger.info('Input rejected by Gemini gate (not a business description)')
          const customTip = await getGeminiHelpTip(sanitized, genAI)
          return respond.ok(res, 200, { recommendations: [], message: customTip || FALLBACK_HELP_TIP, geminiReview: null })
        }
      }
      ...
      if (modelResult) {
        if (modelResult.recommendations.length > 0) {
          ...
          if (unique.length > 0) {
            logger.info('LOB recommendation served by trained model', { count: unique.length })
            return respond.ok(res, 200, { recommendations: unique, source: 'trained_model', geminiReview })
          }
        }
      ...
      if (!apiKey) {
        ...
        return respond.error(res, 503, 'ai_unavailable', 'AI service is not configured. Please add your line of business manually.')
      }
      ...
      try {
        ...
        logger.warn('Gemini response.text() failed (blocked or empty)', { error: textErr.message })
        return respond.error(res, 502, 'ai_parse_error', 'AI returned an unexpected response. Please try again or add your line of business manually.')
      }
      if (!text || typeof text !== 'string') {
        return respond.error(res, 502, 'ai_parse_error', 'AI returned an unexpected response. Please try again or add your line of business manually.')
      }
      ...
      try {
        ...
        logger.warn('Failed to parse Gemini response', { responseLength: text.length, error: parseErr.message })
        return respond.error(res, 502, 'ai_parse_error', 'AI returned an unexpected response. Please try again or add your line of business manually.')
      }
      ...
      return respond.ok(res, 200, { recommendations: unique, source: 'gemini', geminiReview })
    } catch (err) {
      if (err.message?.includes('429') || err.message?.includes('quota')) {
        logger.warn('Gemini rate limit hit', { error: err.message })
        return respond.error(res, 429, 'ai_rate_limit', 'AI service is busy. Please wait a moment and try again, or add your line of business manually.')
      }
      logger.error('AI recommendation failed', { error: err.message, stack: err.stack, name: err.name })
      return respond.error(res, 500, 'ai_error', 'Failed to get AI recommendations. Please try again or add your line of business manually.')
    }
  }
)
  ...
  async (req, res) => {
    try {
      ...
      if (!recommendations || !Array.isArray(recommendations) || recommendations.length === 0) {
        return respond.error(res, 400, 'invalid_input', 'recommendations is required and must be a non-empty array')
      }

      const items = recommendations.map(validateFeedbackRecommendation).filter(Boolean)
      if (items.length === 0) {
        return respond.error(res, 400, 'invalid_input', 'Each recommendation must have taxCode, lineOfBusiness, detailedLine, and accepted')
      }
      ...
      logger.info('LOB recommendation feedback recorded', { id: doc._id, count: items.length })
