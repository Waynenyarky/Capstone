...
async function findProfileAndBusinessByIdentifier(identifier) {
  ...
  const directProfile = await BusinessProfile.findOne(
    buildBusinessLookupQuery(target),
  );
  if (directProfile) {
    const directBusiness = directProfile.businesses.find((b) =>
      isBusinessMatch(b, target),
    );
    if (directBusiness) {
...
function formatRetirementItem(profile, business) {
  const retirementStatus =
    ...
    (business.businessStatus === "closed" ? "confirmed" : "");
  ...
  return {
    ...
    swornStatementGrossSalesDocCid:
      business.swornStatementGrossSalesDocCid || null,
    cessationTaxAssessment: business.cessationTaxAssessment || null,
    businessPlateNo: business.businessPlateNo || "",
    businessAddress:
      business.formData?.businessAddress || business.businessAddress || "",
...
router.post("/:businessId/retire", requireJwt, async (req, res) => {
  try {
    ...
    if (!business)
      return res
        .status(404)
        .json({ error: { code: "NOT_FOUND", message: "Business not found" } });
    ...
    if (
      ...
      return res.status(409).json({
        error: {
          code: "RETIREMENT_ALREADY_PENDING",
          message: "Retirement already requested or confirmed",
        },
      });
    }
    ...
    if (business.businessStartDate || business.yearEstablished) {
      const startYear =
        ...
        new Date(business.businessStartDate).getFullYear();
    ...
    return res.status(500).json({
      error: { code: "INTERNAL", message: "Failed to submit retirement" },
    });
  }
});
...
router.put(
  "/:businessId/review",
  requireJwt,
  requireRole(["lgu_officer", "lgu_manager", "staff", "admin"]),
  async (req, res) => {
    try {
      const { businessId } = req.params;
      ...
      const { profile, business } =
        await findProfileAndBusinessByIdentifier(businessId);
      if (!profile)
      ...
      if (
        business.reviewedBy &&
        String(business.reviewedBy) !== String(req._userId) &&
        !isManagerOrAdmin(req._userRole)
      ) {
        return res.status(403).json({
          error: {
            code: "FORBIDDEN",
            message:
              "Only the claiming officer can review this cessation request",
          },
        });
      }

      if (
        !["requested", "inspector_verified"].includes(
          business.retirementStatus || "",
        )
      ) {
        return res.status(400).json({
          error: {
            code: "INVALID_STATE",
            message: "Cessation request is no longer in a reviewable state",
          },
        });
      }
      ...
      if (decision === "confirmed") {
        ...
        business.retirementRejectionReason =
          ...
          "Cessation request was rejected by the reviewing officer";
      }

      await profile.save();
      return res.json({
        success: true,
        application: formatRetirementItem(profile, business),
      });
    } catch (err) {
      ...
      return res.status(500).json({
        error: {
          code: "INTERNAL",
          message: "Failed to process cessation request",
        },
      });
    }
  },
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        return res.status(409).json({
          error: {
            ...
            message: "Cessation request is already claimed by another officer",
          },
        });
      }
      ...
      await crossClaimForBusiness(bizId, req._userId, {
        skipModel: "Retirement",
      });
      ...
      return res.status(500).json({
        error: {
          code: "INTERNAL",
          message: "Failed to claim cessation request",
        },
      });
    }
  },
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        return res.status(403).json({
          error: {
            code: "FORBIDDEN",
            message:
              "Only the claiming officer can release this cessation request",
          },
        });
      }
      ...
      return res.json({
        success: true,
        application: formatRetirementItem(profile, business),
        message: "Cessation request released",
      });
    } catch (err) {
      ...
      return res.status(500).json({
        error: {
          code: "INTERNAL",
          message: "Failed to release cessation request",
        },
      });
    }
  },
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        return res.status(403).json({
          error: {
            code: "FORBIDDEN",
            message:
              "Only the claiming officer can transfer this cessation request",
          },
        });
      }
      ...
      return res.json({
        success: true,
        application: formatRetirementItem(profile, business),
        message: "Cessation request transferred",
      });
    } catch (err) {
      ...
      return res.status(500).json({
        error: {
          code: "INTERNAL",
          message: "Failed to transfer cessation request",
        },
      });
    }
  },
    ...
    try {
      ...
      if (business.retirementStatus !== "requested") {
        return res.status(400).json({
          error: {
            code: "INVALID_STATE",
            message: "Business must be in requested state",
          },
        });
      }
      ...
      if (verified) {
        ...
        business.retirementStatus = "rejected";
        business.retirementRejectionReason =
          rejectionReason || "Inspector found business still operating";
      ...
      await profile.save();
      return res.json({
        data: { businessId, retirementStatus: business.retirementStatus },
      });
    } catch (err) {
      console.error("POST /retire/verify error:", err);
      return res.status(500).json({
        error: { code: "INTERNAL", message: "Failed to verify retirement" },
      });
    }
  },
);
 ...
 */
router.get(
  "/:businessId/tax-estimate",
  requireJwt,
  requireRole(["lgu_officer", "admin", "lgu_manager", "staff"]),
  async (req, res) => {
    try {
      const { businessId } = req.params;
      const { profile, business } =
      ...
      const lob =
        business.formData?.lineOfBusiness || business.lineOfBusiness || "";
      let feeConfig = null;
      if (lob) {
        feeConfig = await FeeConfiguration.findOne({
          lineOfBusiness: lob,
          isActive: true,
        }).lean();
      }
      if (!feeConfig && business.formData?.taxCode) {
        feeConfig = await FeeConfiguration.findOne({
          taxCode: business.formData.taxCode,
          isActive: true,
        }).lean();
      }
      ...
      const proRatedAmount =
        Math.round((monthsActive / 12) * estimatedAnnualLbt * 100) / 100;
      ...
      return res.status(500).json({
        error: { code: "INTERNAL", message: "Failed to estimate tax" },
      });
    }
  },
);
    ...
    try {
      ...
      if (business.retirementStatus !== "inspector_verified") {
        return res.status(400).json({
          error: {
            code: "INVALID_STATE",
            message: "Business must be inspector-verified to assess tax",
          },
        });
      }

      const totalAmount =
        (lbtAmount || 0) + (surcharges || 0) + (outstandingFees || 0);
      ...
      if (totalAmount <= 0) {
        ...
        return res.json({
          data: {
            ...
            totalAmount: 0,
            message: "Zero tax assessed. Business can be confirmed.",
          },
        });
      }
      ...
      return res.status(500).json({
        error: {
          code: "INTERNAL",
          message: "Failed to assess cessation tax",
        },
      });
    }
  },
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (
        !["inspector_verified", "pending_tax_payment"].includes(
          business.retirementStatus,
        )
      ) {
        return res.status(400).json({
          error: {
            code: "INVALID_STATE",
            message:
              "Business must be inspector-verified or pending tax payment",
          },
        });
      }
      ...
      if (business.retirementStatus === "pending_tax_payment") {
        ...
        if (unpaidCount > 0) {
          return res.status(400).json({
            error: {
              code: "UNPAID_TAX",
              message: `${unpaidCount} cessation tax payment(s) still unpaid`,
            },
          });
        }
      }
      ...
      return res.status(500).json({
        error: { code: "INTERNAL", message: "Failed to confirm retirement" },
      });
    }
  },
);
...
router.get("/", requireJwt, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      error: { code: "INTERNAL", message: "Failed to fetch retirements" },
    });
  }
});
