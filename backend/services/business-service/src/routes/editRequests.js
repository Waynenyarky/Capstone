...
function isBusinessMatch(business, identifier) {
  const target = String(identifier || '')
  return String(business?.businessId || '') === target || String(business?._id || '') === target
}
...
function ensureFormDataObject(business) {
  if (!business.formData || typeof business.formData !== 'object' || Array.isArray(business.formData)) {
    business.formData = {}
  }
}

function applyFieldUpdateToBusiness(business, fieldName, requestedValue) {
  const normalizedValue = typeof requestedValue === 'string' ? requestedValue.trim() : requestedValue
  ...
  switch (fieldName) {
    case 'businessName':
      ...
      business.formData['Business / trade name'] = String(normalizedValue || '')
      break
...
async function applyApprovedEditRequest(editRequest) {
  let profile = await BusinessProfile.findOne(buildBusinessLookupQuery(editRequest.businessId))
  if (!profile && editRequest.requestedBy) {
    // Fallback for legacy/migrating records where querying by businesses.businessId may miss.
    profile = await BusinessProfile.findOne({ userId: editRequest.requestedBy })
  }
  if (!profile) {
    throw new Error('Business profile not found')
  }

  let businessIndex = profile.businesses.findIndex((b) => isBusinessMatch(b, editRequest.businessId))
  ...
  applyFieldUpdateToBusiness(business, editRequest.fieldName, editRequest.requestedValue)
...
router.post('/', requireJwt, async (req, res) => {
  try {
    ...
    const requesterProfile = await BusinessProfile.findOne({ userId: req._userId })
    if (!requesterProfile) {
      return res.status(404).json({
        error: { code: 'PROFILE_NOT_FOUND', message: 'Business profile not found for requesting user' },
      })
    }

    const targetBusiness = requesterProfile.businesses.find((b) => isBusinessMatch(b, businessId))
    if (!targetBusiness) {
      return res.status(404).json({
        error: { code: 'BUSINESS_NOT_FOUND', message: 'Business not found in requester profile' },
      })
    }

    const normalizedBusinessId = String(targetBusiness.businessId || targetBusiness._id || '')
    if (!normalizedBusinessId) {
      return res.status(400).json({
        error: { code: 'INVALID_BUSINESS_ID', message: 'Could not resolve a valid business identifier' },
      })
    }
...
router.put('/:id', requireJwt, requireRole(['lgu_officer', 'staff', 'lgu_manager', 'admin']), async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(403).json({
        error: { code: 'FORBIDDEN', message: 'Only the claiming officer can review this edit request' },
      })
    }
...
router.put('/:id/claim', requireJwt, requireRole(['lgu_officer', 'staff', 'lgu_manager', 'admin']), async (req, res) => {
  try {
    ...
    if (editRequest.status === 'approved' || editRequest.status === 'rejected') {
      return res.status(400).json({
        error: { code: 'ALREADY_RESOLVED', message: 'Cannot claim a resolved edit request' },
      })
    }

    if (editRequest.reviewedBy && String(editRequest.reviewedBy) !== String(req._userId)) {
      return res.status(409).json({
        error: { code: 'ALREADY_CLAIMED', message: 'Edit request is already claimed by another officer' },
      })
    }
    ...
    await crossClaimForBusiness(editRequest.businessId, req._userId, { skipModel: 'EditRequest', skipId: editRequest._id })

    return res.json({ success: true, application: editRequest })
  } catch (err) {
    console.error('PUT /edit-requests/:id/claim error:', err)
    return res.status(500).json({ error: { code: 'INTERNAL', message: 'Failed to claim edit request' } })
  }
})
...
router.put('/:id/release', requireJwt, requireRole(['lgu_officer', 'staff', 'lgu_manager', 'admin']), async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(403).json({
        error: { code: 'FORBIDDEN', message: 'Only the claiming officer can release this edit request' },
      })
    }
    ...
    await crossClaimForBusiness(editRequest.businessId, null, { skipModel: 'EditRequest', skipId: editRequest._id })

    return res.json({ success: true, application: editRequest, message: 'Edit request released' })
  } catch (err) {
    console.error('PUT /edit-requests/:id/release error:', err)
    return res.status(500).json({ error: { code: 'INTERNAL', message: 'Failed to release edit request' } })
  }
})
...
router.put('/:id/transfer', requireJwt, requireRole(['lgu_officer', 'staff', 'lgu_manager', 'admin']), async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(403).json({
        error: { code: 'FORBIDDEN', message: 'Only the claiming officer can transfer this edit request' },
      })
    }
    ...
    await crossClaimForBusiness(editRequest.businessId, targetOfficerId, { skipModel: 'EditRequest', skipId: editRequest._id })

    return res.json({ success: true, application: editRequest, message: 'Edit request transferred' })
  } catch (err) {
    console.error('PUT /edit-requests/:id/transfer error:', err)
    return res.status(500).json({ error: { code: 'INTERNAL', message: 'Failed to transfer edit request' } })
  }
})
