...
router.post("/issue", requireJwt, async (req, res) => {
  try {
    ...
    const permit = await permitService.issuePermit(
      businessId,
      permitType || "initial",
      req._userId,
    );
    ...
    return res.status(500).json({
      error: {
        code: "ISSUANCE_ERROR",
        message: error.message || "Failed to issue permit",
      },
    });
  }
});
...
router.get("/", requireJwt, async (req, res) => {
  try {
    ...
    const permits = await permitService.getBusinessPermits(
      businessId,
      req._userId,
    );
...
router.get("/:permitId/download", requireJwt, async (req, res) => {
  try {
    const pdfBuffer = await permitService.generatePermitPDF(
      req.params.permitId,
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=permit-${req.params.permitId}.pdf`,
    );
    res.send(pdfBuffer);
  } catch (error) {
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const permit = await permitService.suspendPermit(
        req.params.permitId,
        reason,
        req._userId,
      );
      ...
      return res.status(500).json({
        error: {
          ...
          message: error.message || "Failed to suspend permit",
        },
      });
    }
  },
    ...
    try {
      ...
      return res.status(500).json({
        error: {
          ...
          message: error.message || "Failed to revoke permit",
        },
      });
    }
  },
