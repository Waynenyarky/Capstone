...
function validateExample(body) {
  ...
  if (!businessDescription || typeof businessDescription !== 'string' || businessDescription.trim().length < 10) {
...
router.get('/examples', ...adminOnly, async (req, res) => {
  try {
    ...
    const [examples, total] = await Promise.all([
      LobTrainingExample.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
    ...
    logger.error('Failed to list LOB training examples', { error: err.message })
    return respond.error(res, 500, 'server_error', 'Failed to list training examples')
  }
})
...
router.get('/examples/export', ...adminOnly, async (req, res) => {
  try {
    ...
    const examples = await LobTrainingExample.find().sort({ createdAt: -1 }).limit(limit).lean()
    if (format === 'csv') {
      const header = 'businessDescription,taxCode,lineOfBusiness,detailedLine,psicCode'
      const escape = (s) => {
        const t = String(s ?? '').replace(/"/g, '""')
        return t.includes(',') || t.includes('"') || t.includes('\n') ? `"${t}"` : t
      }
      const rows = examples.map((e) =>
        [escape(e.businessDescription), e.taxCode, e.lineOfBusiness || '', e.detailedLine, e.psicCode || ''].join(',')
      )
      const csv = [header, ...rows].join('\n')
      return respond.ok(res, 200, { csv, filename: 'lob-training-examples.csv' })
    }
    return respond.ok(res, 200, { examples, total: examples.length })
  } catch (err) {
    logger.error('Failed to export LOB training examples', { error: err.message })
...
router.post('/examples/import', ...adminOnly, upload.single('file'), async (req, res) => {
  try {
    ...
    if (lines.length < 2) {
      return respond.error(res, 400, 'validation_error', 'CSV must have a header row and at least one data row')
    }
    ...
    if (descIdx === -1 || taxIdx === -1 || dlIdx === -1) {
      return respond.error(res, 400, 'validation_error', 'CSV must have columns: businessDescription, taxCode, detailedLine')
    }
    ...
    for (let i = 0; i < dataLines.length; i++) {
      ...
      const lob = LINE_OF_BUSINESS.find((l) => l.taxCode === taxCode)
      if (!lob || !detailedLine || !lob.detailedLines.includes(detailedLine)) {
    ...
    logger.error('Failed to import LOB training examples', { error: err.message })
    return respond.error(res, 500, 'server_error', err.message || 'Failed to import')
  }
})
...
router.post('/examples', ...adminOnly, async (req, res) => {
  try {
    ...
    logger.error('Failed to create LOB training example', { error: err.message })
    return respond.error(res, 500, 'server_error', 'Failed to create training example')
  }
})
...
router.put('/examples/:id', ...adminOnly, async (req, res) => {
  try {
    ...
    logger.error('Failed to update LOB training example', { error: err.message })
    return respond.error(res, 500, 'server_error', 'Failed to update training example')
  }
})
...
router.delete('/examples/:id', ...adminOnly, async (req, res) => {
  try {
    ...
    logger.error('Failed to delete LOB training example', { error: err.message })
    return respond.error(res, 500, 'server_error', 'Failed to delete training example')
  }
})
...
router.post('/train', ...adminOnly, async (req, res) => {
  try {
    ...
    if (examples.length < 10) {
      return respond.error(res, 400, 'insufficient_data', `Need at least 10 training examples. Currently have ${examples.length}.`)
    }
    ...
    logger.info('Triggering LOB model training', { examples: examples.length, descriptions: dataset.length })
    ...
    if (!trainRes.ok) {
      ...
      logger.error('Model training service returned error', { status: trainRes.status, body })
      return respond.error(res, 502, 'training_failed', body.error || 'Model training service returned an error')
    }

    const result = await trainRes.json()
    logger.info('LOB model training completed', { result })

    return respond.ok(res, 200, { success: true, message: 'Model retrained successfully', trainingExamples: examples.length })
  } catch (err) {
    if (err.name === 'AbortError') {
      return respond.error(res, 504, 'training_timeout', 'Model training took too long. Try again or check the Python service.')
    }
    logger.error('Failed to trigger model training', { error: err.message, stack: err.stack })
    return respond.error(res, 500, 'server_error', 'Failed to trigger model training')
  }
})
  ...
  try {
    ...
    if (modelServiceUrl) {
      try {
        ...
        const hRes = await fetch(`${modelServiceUrl}/health`, { signal: controller.signal })
        clearTimeout(timeout)
    ...
    return respond.error(res, 500, 'server_error', 'Failed to get trainer stats')
  }
})
...
router.get('/audit', ...adminOnly, async (req, res) => {
  try {
    ...
    const aggregated = await LobTrainingExample.aggregate([
      { $group: { _id: { taxCode: '$taxCode', detailedLine: '$detailedLine' }, count: { $sum: 1 } } },
    ])
    const countByLabel = {}
...
router.get('/evaluate', ...adminOnly, async (req, res) => {
  try {
    ...
    if (!evalRes.ok) {
      ...
      return respond.error(res, evalRes.status === 503 ? 503 : 502, 'evaluation_unavailable', body.error || 'Model evaluation failed or model not loaded.')
    }
    const data = await evalRes.json()
    return respond.ok(res, 200, data)
  } catch (err) {
    if (err.name === 'AbortError') {
      return respond.error(res, 504, 'evaluation_timeout', 'Evaluation took too long.')
    }
...
router.get('/gemini-status', ...adminOnly, async (req, res) => {
  try {
    ...
    if (!apiKey || !apiKey.trim()) {
      return respond.ok(res, 200, { configured: false, ok: false, reason: 'not_configured', message: 'GEMINI_API_KEY is not set' })
    }
    ...
    if (text) {
      return respond.ok(res, 200, { configured: true, ok: true, model: GEMINI_LOB_MODEL })
    }
    return respond.ok(res, 200, { configured: true, ok: false, reason: 'empty_response', model: GEMINI_LOB_MODEL })
  } catch (err) {
    const msg = err.message || ''
    let reason = 'error'
    if (msg.includes('429') || msg.includes('quota') || msg.includes('Too Many Requests')) reason = 'quota_exceeded'
    else if (msg.includes('404') || msg.includes('not found')) reason = 'model_not_found'
