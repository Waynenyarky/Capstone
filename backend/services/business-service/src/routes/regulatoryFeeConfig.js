...
router.put('/', requireJwt, requireRole(['admin']), requireAdminStepUp, async (req, res) => {
  try {
    ...
    if (businessPlate != null && typeof businessPlate === 'object') {
      const bp = {}
      if (validNumber(businessPlate.feePerUnit) && businessPlate.feePerUnit >= 0) {
        bp.feePerUnit = businessPlate.feePerUnit
      }
      if (typeof businessPlate.note === 'string') bp.note = businessPlate.note
      if (Object.keys(bp).length > 0) update.businessPlate = bp
    }

    if (weightsAndMeasures != null && typeof weightsAndMeasures === 'object') {
      const wam = {}
      const bracketCheck = (b) => (b.maxValue == null || validNumber(b.maxValue)) && validNumber(b.feePerUnit)
      ...
      if (validNumber(weightsAndMeasures.retestingPerUnit) && weightsAndMeasures.retestingPerUnit >= 0) {
        wam.retestingPerUnit = weightsAndMeasures.retestingPerUnit
      }
      if (validNumber(weightsAndMeasures.gasolinePerNozzle) && weightsAndMeasures.gasolinePerNozzle >= 0) {
    ...
    if (communityTax != null && typeof communityTax === 'object') {
      ...
      ctFields.forEach((f) => {
        if (validNumber(communityTax[f]) && communityTax[f] >= 0) ct[f] = communityTax[f]
      })
    ...
    if (specialPermit != null && typeof specialPermit === 'object') {
      const sp = {}
      if (validNumber(specialPermit.streamerPerSqYard) && specialPermit.streamerPerSqYard >= 0) {
        sp.streamerPerSqYard = specialPermit.streamerPerSqYard
      }
      if (validNumber(specialPermit.streamerDays) && specialPermit.streamerDays >= 0) {
        sp.streamerDays = specialPermit.streamerDays
      }
      if (validNumber(specialPermit.motorcadePerDay) && specialPermit.motorcadePerDay >= 0) {
        sp.motorcadePerDay = specialPermit.motorcadePerDay
      }
      if (Object.keys(sp).length > 0) update.specialPermit = sp
    }

    if (certificationOfBusinessRecord != null && typeof certificationOfBusinessRecord === 'object') {
      const cert = {}
      if (validNumber(certificationOfBusinessRecord.fee) && certificationOfBusinessRecord.fee >= 0) {
      ...
      if (
        ...
        cert.documentaryStamp = certificationOfBusinessRecord.documentaryStamp
      }
      if (Object.keys(cert).length > 0) update.certificationOfBusinessRecord = cert
    }

    if (certifiedTrueCopyPerDocument != null && typeof certifiedTrueCopyPerDocument === 'object') {
      const cpy = {}
      if (validNumber(certifiedTrueCopyPerDocument.fee) && certifiedTrueCopyPerDocument.fee >= 0) {
