  ...
  try {
    ...
    if (gpsAtStart && typeof gpsAtStart.lat === 'number' && typeof gpsAtStart.lng === 'number') {
      ...
      if (geo && typeof geo.lat === 'number' && typeof geo.lng === 'number') {
        const dist = haversineDistanceMeters(gpsAtStart.lat, gpsAtStart.lng, geo.lat, geo.lng)
    ...
    return res.json({ success: true, inspection, gpsMismatch: inspection.gpsMismatch })
  } catch (err) {
