...
const GPS_MISMATCH_THRESHOLD_METERS = Number(
  process.env.GPS_MISMATCH_THRESHOLD_METERS || 500,
);
...
const evidenceUploadsRoot = path.join(
  __dirname,
  "..",
  "..",
  "..",
  ...
  try {
    ...
    const businessProfile = inspection.businessProfileId
      ? {
          ...
          address:
            ...
            [business?.street, business?.barangay, business?.cityMunicipality]
              .filter(Boolean)
              .join(", "),
    ...
    return res.json({
      ...
      businessProfile: businessProfile
        ? { ...businessProfile, openViolations }
        : null,
      assignedBy: inspection.assignedBy
        ? {
            name: `${inspection.assignedBy.firstName} ${inspection.assignedBy.lastName}`,
            email: inspection.assignedBy.email,
          }
        : null,
    });
  } catch (err) {
    ...
    return respond.error(
      ...
      500,
      "fetch_error",
      err.message || "Failed to fetch inspection",
    );
  }
});
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const [repeatViolationsCount, pastFailedCount, pendingAppealsCount] =
        await Promise.all([
      ...
      console.error(
        "GET /api/inspector/inspections/:id/risk-indicators error:",
        err,
      );
      return respond.error(
        res,
        500,
        "fetch_error",
        err.message || "Failed to fetch risk indicators",
      );
    }
  },
);
    ...
    try {
      ...
      if (inspection.status !== "pending")
        return respond.error(
          ...
          400,
          "invalid_state",
          "Inspection can only be started when pending",
        );

      let gpsMismatch = false;
      if (
        gpsAtStart &&
        typeof gpsAtStart.lat === "number" &&
        typeof gpsAtStart.lng === "number"
      ) {
        inspection.gpsAtStart = {
          lat: gpsAtStart.lat,
          lng: gpsAtStart.lng,
          accuracy:
            typeof gpsAtStart.accuracy === "number"
              ? gpsAtStart.accuracy
              : null,
          capturedAt: gpsAtStart.capturedAt
            ? new Date(gpsAtStart.capturedAt)
            : new Date(),
        };
        ...
        if (geo && typeof geo.lat === "number" && typeof geo.lng === "number") {
          ...
          if (dist > GPS_MISMATCH_THRESHOLD_METERS) {
            ...
            inspection.gpsMismatchReason =
              typeof gpsMismatchReason === "string"
                ? gpsMismatchReason.trim()
                : "";
          }
        }
      }
      ...
      return respond.error(
        ...
        500,
        "update_error",
        err.message || "Failed to start inspection",
      );
    }
  },
);
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (!inspection)
        return respond.error(res, 404, "not_found", "Inspection not found");
      if (!inspection.gpsMismatch)
        return respond.error(
          res,
          400,
          "invalid_state",
          "No GPS mismatch to document",
        );
      ...
      inspection.gpsMismatchReason =
        typeof gpsMismatchReason === "string" ? gpsMismatchReason.trim() : "";
      await inspection.save();

      return res.json({ success: true, inspection });
    } catch (err) {
      console.error(
        "PATCH /api/inspector/inspections/:id/gps-mismatch-reason error:",
        err,
      );
      return respond.error(
        res,
    ...
    try {
      ...
      if (inspection.status !== "in_progress")
        return respond.error(
          ...
          400,
          "invalid_state",
          "Checklist can only be updated when inspection is in progress",
        );
      if (inspection.isImmutable)
        return respond.error(
          ...
          400,
          "immutable",
          "Inspection has been submitted and cannot be modified",
        );
    ...
    try {
      ...
      if (!violationType || !description || !severity || !complianceDeadline) {
        return respond.error(
          ...
          400,
          "missing_fields",
          "violationType, description, severity, and complianceDeadline are required",
        );
      }
      ...
      if (inspection.status !== "in_progress")
        return respond.error(
          ...
          400,
          "invalid_state",
          "Violations can only be issued when inspection is in progress",
        );
      if (inspection.isImmutable)
        return respond.error(
          res,
      ...
      blockchainQueue.queueBlockchainOperation("logAuditHash", [
        hash,
        "violation_issued",
      ]);
      logAuditEvent(
        ...
        { violationId, inspectionId: id, severity },
      );

      return res.status(201).json({ success: true, violation });
    } catch (err) {
      console.error(
        "POST /api/inspector/inspections/:id/violations error:",
        err,
      );
      return respond.error(
        res,
        500,
        "create_error",
        err.message || "Failed to issue violation",
      );
    }
  },
);
    ...
    try {
      ...
      if (inspection.status !== "in_progress")
        return respond.error(
          ...
          400,
          "invalid_state",
          "Evidence can only be added when inspection is in progress",
        );
      if (inspection.isImmutable)
        return respond.error(
          res,
      ...
      const evidenceItem = {
        ...
        metadata: {
          ...
          annotationX:
            typeof meta.annotationX === "number" ? meta.annotationX : undefined,
          annotationY:
            typeof meta.annotationY === "number" ? meta.annotationY : undefined,
        },
      };
      ...
      return respond.error(
        ...
        500,
        "upload_error",
        err.message || "Failed to upload evidence",
      );
    }
  },
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (
        !overallResult ||
        !["passed", "failed", "needs_reinspection"].includes(overallResult)
      ) {
        return respond.error(
          res,
          400,
          "invalid_result",
          "overallResult must be passed, failed, or needs_reinspection",
        );
      }
      const inspection = await Inspection.findOne({ _id: id, inspectorId });
      if (!inspection)
      ...
      if (
        inspectorSignature &&
        typeof inspectorSignature.dataUrl === "string" &&
        inspectorSignature.dataUrl
      ) {
        inspection.inspectorSignature = {
          dataUrl: inspectorSignature.dataUrl,
          timestamp: inspectorSignature.timestamp
            ? new Date(inspectorSignature.timestamp)
            : now,
        };
      }
      ...
      const hashPayload = JSON.stringify({
        ...
        inspectorSignature: inspection.inspectorSignature?.dataUrl
          ? inspection.inspectorSignature.dataUrl.substring(0, 64)
          : "",
      });
      const hash = crypto
        .createHash("sha256")
      ...
      try {
        const businessProfile = await BusinessProfile.findById(
          ...
          .lean();
        const business = (businessProfile?.businesses || []).find(
          (b) => b.businessId === inspection.businessId,
        );
        const businessName =
          business?.businessName ||
        ...
        console.warn(
          "Failed to notify business owner about completed inspection:",
          notificationErr,
        );
      }
      ...
      logAuditEvent(
        ...
        { overallResult, businessId: inspection.businessId },
      );
      ...
      return respond.error(
        ...
        500,
        "submit_error",
        err.message || "Failed to submit inspection",
      );
    }
  },
);
