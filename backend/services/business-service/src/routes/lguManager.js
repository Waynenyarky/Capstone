...
router.get("/analytics/dashboard", async (req, res) => {
  try {
    ...
    if (
      approvedWithTimes[0]?.count > 0 &&
      approvedWithTimes[0]?.totalMs != null
    ) {
      averageProcessingTimeDays =
        approvedWithTimes[0].totalMs /
        approvedWithTimes[0].count /
        (24 * 60 * 60 * 1000);
    }
  ...
  try {
    ...
    if (search) {
      matchStage.$or = [
        ...
        {
          "businesses.applicationReferenceNumber": {
            $regex: search,
            $options: "i",
          },
        },
    ...
    return respond.error(
      ...
      500,
      "permits_error",
      err.message || "Failed to fetch permits overview",
    );
  }
});
...
router.get("/overview/violations-inspections", async (req, res) => {
  try {
    ...
    const [violationByStatus, inspectionByStatus, overdueCount] =
      await Promise.all([
        Violation.aggregate([
          { $group: { _id: "$status", count: { $sum: 1 } } },
        ]),
    ...
    console.error(
      "GET /api/lgu-manager/overview/violations-inspections error:",
      err,
    );
    return respond.error(
      res,
...
router.get("/overview/cessations", async (req, res) => {
  try {
    ...
    const normalizedProjection = {
      ...
      retirementStatus: {
        $cond: [
          ...
          {
            $cond: [
              { $eq: ["$businesses.businessStatus", "closed"] },
              "confirmed",
              "",
            ],
          },
    ...
    return respond.error(
      ...
      500,
      "cessations_error",
      err.message || "Failed to fetch cessations overview",
    );
  }
});
...
router.get("/overview/appeals", async (req, res) => {
  try {
    ...
    return respond.error(
      ...
      500,
      "appeals_error",
      err.message || "Failed to fetch appeals overview",
    );
  }
});
...
router.post("/reports/generate", async (req, res) => {
  try {
    ...
    const [permitsByStatus, violationsCount, inspectionsCount] =
      await Promise.all([
        BusinessProfile.aggregate([
          { $unwind: "$businesses" },
          ...
          {
            $group: {
              _id: "$businesses.applicationStatus",
              count: { $sum: 1 },
            },
          },
        ]),
...
router.put("/payments/:paymentId/verify", async (req, res) => {
  try {
    ...
    console.error(
      "PUT /api/lgu-manager/payments/:paymentId/verify error:",
      err,
    );
    return respond.error(
      res,
...
router.put("/payments/:paymentId/reject", async (req, res) => {
  try {
    ...
    console.error(
      "PUT /api/lgu-manager/payments/:paymentId/reject error:",
      err,
    );
    return respond.error(
      res,
...
router.get("/inspections/pending", async (req, res) => {
  try {
    ...
    return respond.error(
      ...
      500,
      "fetch_error",
      "Failed to fetch pending inspections",
    );
  }
});
...
router.get("/inspectors", async (req, res) => {
  try {
    ...
    const inspectors = await User.find({ role: inspectorRole._id }).select(
...
router.get("/inspectors/:inspectorId/workload", async (req, res) => {
  try {
    ...
    return respond.success(res, 200, {
      ...
      pendingInspections,
      totalInspections:
        activeInspections + completedInspections + pendingInspections,
    });
  } catch (err) {
    console.error(
      "GET /api/lgu-manager/inspectors/:inspectorId/workload error:",
      err,
    );
    return respond.error(
      res,
      500,
      "fetch_error",
      "Failed to fetch inspector workload",
    );
  }
});
...
router.get("/inspectors/:inspectorId/schedule", async (req, res) => {
  try {
    ...
    console.error(
      "GET /api/lgu-manager/inspectors/:inspectorId/schedule error:",
      err,
    );
    return respond.error(
      res,
      500,
      "fetch_error",
      "Failed to fetch inspector schedule",
    );
  }
});
