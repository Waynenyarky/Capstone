...
router.get('/', requireJwt, requireRole(['business_owner']), async (req, res) => {
  try {
    ...
    const inspections = await Inspection.find(inspectionFilter).select('_id businessId').lean()
 ...
 */
router.get('/open', requireJwt, requireRole(['business_owner']), async (req, res) => {
  try {
 ...
 */
router.get('/summary', requireJwt, requireRole(['business_owner']), async (req, res) => {
  try {
    const profile = await BusinessProfile.findOne({ userId: req._userId }).lean()
    if (!profile) {
      return res.json({
    ...
    return res.status(500).json({
      error: { code: 'INTERNAL', message: 'Failed to fetch violation summary' }
    })
  }
})
 ...
 */
router.get('/:violationId', requireJwt, requireRole(['business_owner']), async (req, res) => {
  try {
    ...
    if (!inspection) {
      return res.status(403).json({
        error: { code: 'FORBIDDEN', message: 'Violation does not belong to user' }
      })
    }
    ...
    return res.json({
      data: {
        ...
        isOverdue: violation.status === 'open' && new Date(violation.complianceDeadline) < now,
        daysUntilDeadline: Math.ceil(
          (new Date(violation.complianceDeadline) - now) / (1000 * 60 * 60 * 24)
        ),
 ...
 */
router.post('/:violationId/acknowledge', requireJwt, requireRole(['business_owner']), async (req, res) => {
  try {
    const { violationId } = req.params
