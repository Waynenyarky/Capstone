...
router.get('/', requireJwt, async (req, res) => {
  try {
    ...
    const [permits, total] = await Promise.all([
      OccupationalPermit.find(filter).sort({ createdAt: -1 }).skip(skip).limit(Number(limit)).lean(),
    ...
    return res.status(500).json({ error: { code: 'INTERNAL', message: 'Failed to fetch occupational permits' } })
  }
})
...
router.post('/', requireJwt, async (req, res) => {
  try {
    const {
      firstName, lastName, gender, civilStatus, dateOfBirth, address, education,
      businessPlateNo, employer, company, position, type,
    } = req.body

    if (!businessPlateNo) {
      return res.status(400).json({
        error: { code: 'VALIDATION_ERROR', message: 'businessPlateNo is required' },
      })
    }
    ...
    if (!profile) {
      return res.status(404).json({
        error: { code: 'BP_NUMBER_NOT_FOUND', message: 'Business plate number not found in system' },
      })
    }
    ...
    const permit = await OccupationalPermit.create({
      ...
      firstName, lastName, gender, civilStatus, dateOfBirth, address, education,
      businessPlateNo, employer, company, position,
      type: type || 'employed',
      status: 'pending',
    })
    ...
    return res.status(500).json({ error: { code: 'INTERNAL', message: 'Failed to create occupational permit' } })
  }
})
...
router.put('/:id', requireJwt, async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: { code: 'INTERNAL', message: 'Failed to update' } })
  }
})
