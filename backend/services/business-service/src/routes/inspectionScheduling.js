    ...
    try {
      ...
      if (!slotId || !businessId) {
        return respond.error(
          ...
          400,
          "required_fields",
          "Slot ID and business ID are required",
        );
      }

      // Check prerequisites
      const prerequisites =
        await inspectionSchedulingService.checkPrerequisites(businessId);
      ...
      return respond.error(
        ...
        500,
        "booking_error",
        err.message || "Failed to book inspection",
      );
    }
  },
);
...
router.post("/cancel", requireJwt, async (req, res) => {
  try {
    ...
    return respond.error(
      ...
      500,
      "cancel_error",
      err.message || "Failed to cancel inspection",
    );
  }
});
  ...
  try {
    ...
    if (!oldSlotId || !newSlotId) {
      return respond.error(
        ...
        400,
        "slots_required",
        "Old slot ID and new slot ID are required",
      );
    }
...
router.get("/upcoming/:businessId", requireJwt, async (req, res) => {
  try {
    ...
    const inspections =
      await inspectionSchedulingService.getUpcomingInspections(businessId);
    ...
    return respond.error(
      ...
      500,
      "fetch_error",
      err.message || "Failed to fetch upcoming inspections",
    );
  }
});
...
router.get("/history/:businessId", requireJwt, async (req, res) => {
  try {
    ...
    const history =
      await inspectionSchedulingService.getInspectionHistory(businessId);
    res.json({ history });
  } catch (err) {
    ...
    return respond.error(
      ...
      500,
      "fetch_error",
      err.message || "Failed to fetch inspection history",
    );
  }
});
...
router.get("/prerequisites/:businessId", requireJwt, async (req, res) => {
  try {
    ...
    return respond.error(
      ...
      500,
      "check_error",
      err.message || "Failed to check prerequisites",
    );
  }
});
    ...
    try {
      ...
      if (!inspectorId || !date || !startTimes || !Array.isArray(startTimes)) {
        return respond.error(
          ...
          400,
          "required_fields",
          "Inspector ID, date, and start times are required",
        );
      }
      ...
      return respond.error(
        ...
        500,
        "create_error",
        err.message || "Failed to create inspection slots",
      );
    }
  },
);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      console.error(
        "GET /api/inspections/inspector-schedule/:inspectorId error:",
        err,
      );
      return respond.error(
        res,
        500,
        "fetch_error",
        err.message || "Failed to fetch inspector schedule",
      );
    }
  },
);
...
router.post(
  ...
  async (req, res) => {
    try {
      const { slotId, findings, status, violations, documents, notes } =
        req.body;
      ...
      if (!slotId || !status) {
        return respond.error(
          ...
          400,
          "required_fields",
          "Slot ID and status are required",
        );
      }
      ...
      return respond.error(
        ...
        500,
        "complete_error",
        err.message || "Failed to complete inspection",
      );
    }
  },
);
