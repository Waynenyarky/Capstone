  ...
  async (req, res) => {
    try {
      ...
      if (permitType === "general_permit") {
        ...
        logAuditEvent(
          "walk_in_general_permit_created",
          req._userId,
          "GeneralPermit",
          generalPermit._id.toString(),
          {
      ...
      const newBusiness = {
        ...
        businessName:
          effectiveBusinessData.registeredBusinessName ||
          effectiveBusinessData.businessName ||
          "Walk-in Business",
        businessRegistrationNumber:
          effectiveBusinessData.businessRegistrationNumber || businessId,
        ...
        employeesResidingInLgu:
          effectiveBusinessData.employeesResidingInLgu || 0,
        businessLocationType:
          effectiveBusinessData.businessLocationType || "owned",
        // Owner
        ...
        applicationStatus: businessData
          ? draft
            ? "draft"
            : "submitted"
          : "draft",
      ...
      try {
        computedFees = await computeApplicationFees({
      ...
      return res.status(500).json({
        error: {
          code: "INTERNAL",
          message: "Failed to create walk-in application",
        },
      });
    }
  },
  ...
  async (req, res) => {
    try {
      ...
      const response = await axios
        .get(`${authServiceUrl}/api/auth/users/search`, {
          params: { q },
  ...
  async (req, res) => {
    try {
      ...
      const businessIndex = profile.businesses.findIndex(
        (b) =>
          String(b.businessId) === businessId || String(b._id) === businessId,
      );
      if (businessIndex === -1) {
        return res.status(404).json({
          error: {
            code: "NOT_FOUND",
            message: "Business not found in profile",
          },
        });
      }
      ...
      const isFinalSubmit =
        (businessData.applicationStatus || "").toLowerCase() === "submitted";
      ...
      const existingLgu =
        existing.lguDocuments && typeof existing.lguDocuments === "object"
      ...
      if (isFinalSubmit) {
        ...
        logAuditEvent(
          "walk_in_submitted_and_approved",
          officerId,
          "BusinessProfile",
          profile._id.toString(),
          {
      ...
      if (isFinalSubmit) {
        try {
          ...
          const bName =
            updated.businessName || updated.registeredBusinessName || "N/A";
          const ownerName = updated.ownerFullName || "N/A";
          const address =
            typeof updated.businessAddress === "string"
              ...
              : [
                  ...
                  .join(", ") || "N/A";
          const lob =
            updated.primaryLineOfBusiness || updated.lineOfBusiness || "N/A";
          ...
          console.log(
            `[walk-in] Auto-issued permit ${permitNumber} for business ${bId}`,
          );
        } catch (permitErr) {
          console.warn(
            "[walk-in] Permit auto-issuance failed (non-blocking):",
            permitErr.message,
          );
        }
      }
      ...
      return res.status(500).json({
        error: {
          code: "INTERNAL",
          message: err.message || "Failed to update walk-in application",
        },
      });
    }
  },
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      return res.status(500).json({
        error: { code: "INTERNAL", message: "Failed to compute fees" },
      });
    }
  },
);
