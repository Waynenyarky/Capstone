...
router.post('/gcash', async (req, res) => {
  try {
    ...
    const payment = await Payment.findOne({ referenceNumber: reference_number })
    ...
    // Log audit event
    logAuditEvent('payment_webhook_received', null, 'Payment', payment._id.toString(), {
      provider: 'gcash',
      status,
...
router.post('/bank-transfer', async (req, res) => {
  try {
    ...
    const { reference_number, status, amount, transaction_reference } = req.body
