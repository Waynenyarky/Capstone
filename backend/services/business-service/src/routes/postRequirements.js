...
router.get('/', requireJwt, async (req, res) => {
  try {
    ...
    const [reqs, total] = await Promise.all([
      PostRequirement.find(filter).sort({ dueDate: 1 }).skip(skip).limit(Number(limit)).lean(),
    ...
    return res.json({ data: enriched, meta: { page: Number(page), limit: Number(limit), total } })
  } catch (err) {
    console.error('GET /post-requirements error:', err)
    return res.status(500).json({ error: { code: 'INTERNAL', message: 'Failed to fetch post-requirements' } })
  }
})
...
router.post('/', requireJwt, async (req, res) => {
  try {
    const { businessId, ownerId, permitId, requirementType, description, dueDate } = req.body
    if (!businessId || !requirementType) {
      return res.status(400).json({
        error: { code: 'VALIDATION_ERROR', message: 'businessId and requirementType are required' },
      })
    }
    ...
    if (dueDate && new Date(dueDate) <= new Date()) {
      return res.status(400).json({
        error: { code: 'VALIDATION_ERROR', message: 'dueDate must be in the future' },
      })
    }
    ...
    return res.status(500).json({ error: { code: 'INTERNAL', message: 'Failed to create post-requirement' } })
  }
})
...
router.put('/:id', requireJwt, async (req, res) => {
  try {
    ...
    if (!postReq) {
      return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Post-requirement not found' } })
    }
    ...
    if (status === 'non_compliant') {
      postReq.verifiedBy = req._userId
      postReq.verificationNotes = verificationNotes || 'Non-compliant submission'
    }
    await postReq.save()
    return res.json({ data: postReq })
  } catch (err) {
    console.error('PUT /post-requirements error:', err)
    return res.status(500).json({ error: { code: 'INTERNAL', message: 'Failed to update post-requirement' } })
  }
})
...
router.put('/:id/extend', requireJwt, async (req, res) => {
  try {
    ...
    if (postReq.status === 'verified') {
      return res.status(400).json({
        error: { code: 'ALREADY_VERIFIED', message: 'Cannot extend a verified post-requirement' },
      })
    }
    ...
    if (new Date(newDueDate) <= new Date()) {
      return res.status(400).json({
        error: { code: 'VALIDATION_ERROR', message: 'newDueDate must be in the future' },
      })
    }
    ...
    return res.status(500).json({ error: { code: 'INTERNAL', message: 'Failed to extend due date' } })
  }
})
