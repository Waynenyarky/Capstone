...
async function crossClaimForBusiness(businessId, officerId, options = {}) {
  ...
  try {
    ...
    if (skipModel !== "EditRequest") {
      ...
      const editResult = await EditRequest.updateMany(editFilter, {
    ...
    if (skipModel !== "Appeal") {
      ...
      try {
        const profileForAliases = await BusinessProfile.findOne(
        ...
        if (profileForAliases) {
          ...
          const biz = profileForAliases.businesses.find(
            (b) =>
              String(b.businessId || "") === target ||
              String(b._id || "") === target,
          );
          if (biz) {
            if (
              biz.businessId &&
              !appealBusinessIds.includes(String(biz.businessId))
            )
              appealBusinessIds.push(String(biz.businessId));
            if (biz._id && !appealBusinessIds.includes(String(biz._id)))
              appealBusinessIds.push(String(biz._id));
          }
        }
      } catch (_) {
      ...
      const appealResult = await Appeal.updateMany(appealFilter, {
        $set: { reviewedBy: officerId },
      });
    ...
    if (skipModel !== "Retirement") {
      ...
      if (profile) {
        ...
        if (
          ...
          !["confirmed", "rejected"].includes(business.retirementStatus)
        ) {
