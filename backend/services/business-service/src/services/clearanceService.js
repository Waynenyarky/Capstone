...
const AGENCY_ORDER = ['BARANGAY', 'ZONING', 'SANITARY', 'FIRE_SAFETY', 'BFP', 'TREASURY', 'MAYORS_OFFICE'];
...
async function resolveBusinessProfile(businessId) {
  ...
  let profile = await BusinessProfile.findOne({ 'businesses.businessId': businessId })
...
async function initiateClearance(businessId, applicationId, initiatedBy) {
  ...
  const clearance = new Clearance({
    ...
    notificationsSent: [{
      type: 'PORTAL',
      content: 'Your application has been submitted for clearance processing.',
...
async function getClearanceStatus(businessId) {
  const profile = await resolveBusinessProfile(businessId)
  const clearance = await Clearance.findOne(clearanceBusinessFilter(profile, businessId))
 ...
 */
async function submitToAgency(businessId, agency, { documents, notes, submittedBy }) {
  const profile = await resolveBusinessProfile(businessId)
  const clearance = await Clearance.findOne(clearanceBusinessFilter(profile, businessId));
  if (!clearance) {
 ...
 */
async function approveAgencyClearance(clearanceId, agency, { approvedBy, notes, certificateNumber }) {
  const clearance = await Clearance.findById(clearanceId);
  if (!clearance) {
    throw new Error('Clearance not found');
  }
  ...
  agencyClearance.expiresAt.setFullYear(agencyClearance.expiresAt.getFullYear() + 1);
  ...
  if (nextAgency) {
    ...
    const nextClearance = clearance.clearances.find(c => c.agency === nextAgency);
    if (nextClearance) {
  ...
  clearance.stageHistory.push({
    ...
    duration: Math.round((Date.now() - agencyClearance.reviewStartedAt) / (1000 * 60 * 60)) // hours
  });
 ...
 */
async function rejectAgencyClearance(clearanceId, agency, { rejectedBy, reason }) {
  const clearance = await Clearance.findById(clearanceId);
  if (!clearance) {
    throw new Error('Clearance not found');
  }
 ...
 * Raise deficiency
 */
async function raiseDeficiency(clearanceId, agency, { raisedBy, description, requiredDocuments }) {
  const clearance = await Clearance.findById(clearanceId);
  if (!clearance) {
    throw new Error('Clearance not found');
  }
 ...
 * Resolve deficiency
 */
async function resolveDeficiency(clearanceId, agency, deficiencyId, { resolvedBy, resolutionNotes, uploadedDocuments }) {
  const clearance = await Clearance.findById(clearanceId);
  if (!clearance) {
    throw new Error('Clearance not found');
  }
 ...
 */
async function getAgencyQueue(agency, { status = 'PENDING', page = 1, limit = 20 }) {
  const query = {
...
async function getClearanceTimeline(businessId) {
  ...
  return {
    ...
    totalDuration: clearance.completedAt 
      ? Math.round((new Date(clearance.completedAt) - new Date(clearance.startedAt)) / (1000 * 60 * 60 * 24))
      : null,
    ...
    agencyStages: clearance.clearances.map(c => ({
      ...
      submittedAt: c.submittedAt,
      reviewedAt: c.reviewedAt,
      duration: c.reviewStartedAt && c.reviewedAt
        ? Math.round((new Date(c.reviewedAt) - new Date(c.reviewStartedAt)) / (1000 * 60 * 60))
        : null
    }))
  };
}
 ...
 * Send notification
 */
async function sendNotification(businessId, { type, content, recipientId, sentBy }) {
  const profile = await resolveBusinessProfile(businessId)
  const clearance = await Clearance.findOne(clearanceBusinessFilter(profile, businessId));
  if (!clearance) {
    throw new Error('Clearance not found');
