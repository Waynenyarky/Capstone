...
async function issuePermit(businessId, permitType = "initial", userId) {
  ...
  let business = profile.businesses.find(
  ...
  const address =
    addressParts.length > 0 ? addressParts.join(", ") : "Address not specified";

  const permitData = {
    permitNumber,
    businessId,
    businessName:
      business.businessName || business.registeredBusinessName || "Business",
    ownerName:
      profile.ownerName ||
...
async function getBusinessPermits(businessId, userId = null) {
  ...
  const query =
    aliasList.length > 1
      ? { businessId: { $in: aliasList } }
      : { businessId: aliasList[0] };
...
async function generatePermitPDF(permitId) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc
        .text("Business Name:", leftMargin, y, { continued: true })
        .font("Helvetica-Bold")
      ...
      doc
        ...
        .font("Helvetica-Bold")
        .text(
          ` ${new Date(permit.issuedDate).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}`,
        );
      ...
      doc
        ...
        .font("Helvetica-Bold")
        .text(
          ` ${new Date(permit.expiryDate).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}`,
        );
      ...
      // QR Code
      if (permit.qrCode) {
        const qrImageData = permit.qrCode.replace(
      ...
      doc
        ...
        .text(
          "This permit must be posted in a conspicuous place within the business premises.",
          { align: "center" },
        );
...
async function suspendPermit(permitId, reason, userId) {
  ...
  await BusinessProfile.updateOne(buildBusinessLookupQuery(permit.businessId), {
