...
class WhatIfFeeService {
  ...
  static calculateBaseFees(businessData) {
    ...
    return {
      ...
      totalFees: Math.round(baseFee + sanitaryFee + fireSafetyFee + regulatoryFees)
    }
  }
  ...
  static calculateWhatIfFees(businessData) {
    try {
      ...
      const taxBrackets = this.calculateTaxBrackets(fees.totalFees, businessData.grossAnnualSales)
      ...
      return {
        ...
        recommendations: this.generateOptimizationRecommendations(businessData, fees)
      }
  ...
  static async getFeeImpactAnalysis(businessId, changes) {
    try {
      ...
      const impact = {
        ...
        percentageChange: ((newFees.totalFees - currentFees.totalFees) / currentFees.totalFees * 100).toFixed(2),
        breakdown: {
    ...
    try {
      const results = scenarios.map((scenario, index) => {
        ...
        return {
          ...
          fees,
          taxBrackets: this.calculateTaxBrackets(fees.totalFees, scenario.grossAnnualSales)
        }
      })
      ...
      return {
        ...
        comparison: {
          ...
          difference,
          percentageDifference: ((difference / lowest.fees.totalFees) * 100).toFixed(2)
        },
  ...
  static calculateTaxBrackets(totalFees, grossSales) {
    ...
    if (grossSales <= 100000) {
      ...
      brackets.push({ range: '100K-500K', rate: 0.03, tax: (grossSales - 100000) * 0.03 })
    } else if (grossSales <= 1000000) {
      brackets.push({ range: '0-100K', rate: 0.02, tax: 2000 })
      brackets.push({ range: '100K-500K', rate: 0.03, tax: 12000 })
      brackets.push({ range: '500K-1M', rate: 0.05, tax: (grossSales - 500000) * 0.05 })
    } else {
      brackets.push({ range: '0-100K', rate: 0.02, tax: 2000 })
      brackets.push({ range: '100K-500K', rate: 0.03, tax: 12000 })
      brackets.push({ range: '500K-1M', rate: 0.05, tax: 25000 })
      brackets.push({ range: '1M+', rate: 0.08, tax: (grossSales - 1000000) * 0.08 })
    }
  ...
  static calculatePenalties(businessData) {
    ...
    if (businessData.filingDelayDays > 0) {
      ...
      reasons.push(`Late filing: ${latePenalty} (${businessData.filingDelayDays} days)`)
    }

    // Undercapitalization penalty
    if (businessData.capitalInvestment && businessData.requiredCapital) {
      if (businessData.capitalInvestment < businessData.requiredCapital) {
        const shortfall = businessData.requiredCapital - businessData.capitalInvestment
    ...
    return {
      total: penalties,
      reasons,
      description: penalties > 0 ? 'Penalties assessed for compliance issues' : 'No penalties'
    }
  }
    ...
    switch (feeType) {
      case 'base':
        if (businessData.capitalInvestment > 500000) {
          factors.push(`Capital adjustment: ${businessData.capitalInvestment > 1000000 ? '+50%' : '+20%'}`)
        }
        if (businessData.grossAnnualSales > 1000000) {
          factors.push(`Sales adjustment: ${businessData.grossAnnualSales > 5000000 ? '+30%' : '+15%'}`)
        }
        break
      case 'sanitary':
        if (businessData.numberOfEmployees > 20) {
          factors.push(`Employee adjustment: +${businessData.numberOfEmployees > 50 ? '5' : '2'} per employee`)
        }
        break
      case 'fire':
        if (businessData.businessArea > 500) {
          factors.push(`Area adjustment: +${Math.floor(businessData.businessArea / 100) * 50}`)
        }
        break
    }
  ...
  static generateImpactScenarios(currentData, changes) {
    ...
    if (changes.capitalInvestment && changes.capitalInvestment > currentData.capitalInvestment) {
    ...
    if (changes.grossAnnualSales && changes.grossAnnualSales > currentData.grossAnnualSales) {
  ...
  static generateComparisonInsights(results) {
    ...
    if (results.length > 1) {
      const optimization = results[1].fees.totalFees - results[0].fees.totalFees
      insights.push({
    ...
    return [
      {
        ...
        impact: 'High',
        description: 'Different business types have significantly different base fees'
      },
      {
        factor: 'Location',
        impact: 'Medium',
        description: 'Central business district locations incur 50% higher fees'
      },
      {
