...
async function getAvailableSlots(
  ...
  return slots.map((slot) => ({
    ...
    inspectorName: slot.inspectorId
      ? `${slot.inspectorId.firstName} ${slot.inspectorId.lastName}`
      : null,
    date: slot.date,
...
async function checkPrerequisites(businessId) {
  ...
  const prerequisites = {
    hasClearances: ["clearance_complete", "approved", "active"].includes(
...
async function sendReminders() {
  ...
  for (const slot of slots) {
    ...
    const emailReminderSent = slot.remindersSent?.some(
      (r) => r.type === "EMAIL",
    );
