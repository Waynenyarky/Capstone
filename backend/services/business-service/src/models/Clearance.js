...
const AgencyClearanceSchema = new mongoose.Schema({
  agency: {
    type: String,
    required: true,
    enum: ['ZONING', 'SANITARY', 'FIRE_SAFETY', 'BARANGAY', 'BUILDING', 'BFP', 'TREASURY', 'MAYORS_OFFICE']
  },
  status: {
    type: String,
    enum: ['PENDING', 'UNDER_REVIEW', 'APPROVED', 'REJECTED', 'WAIVED', 'CONDITIONAL'],
    default: 'PENDING'
  },
  reviewerId: {
    type: mongoose.Schema.Types.ObjectId,
...
const ClearanceSchema = new mongoose.Schema({
  ...
  currentAgency: {
    type: String,
    enum: ['ZONING', 'SANITARY', 'FIRE_SAFETY', 'BARANGAY', 'BUILDING', 'BFP', 'TREASURY', 'MAYORS_OFFICE', null],
    default: null
  },
  queuePosition: {
    type: Number,
...
ClearanceSchema.virtual('completionPercentage').get(function() {
  ...
  const approved = this.clearances.filter(c => c.status === 'APPROVED' || c.status === 'WAIVED').length;
...
ClearanceSchema.methods.getNextPendingAgency = function() {
  const agencyOrder = ['BARANGAY', 'ZONING', 'SANITARY', 'FIRE_SAFETY', 'BFP', 'TREASURY', 'MAYORS_OFFICE'];
...
ClearanceSchema.methods.checkAllApproved = function() {
  ...
  const approved = this.clearances.filter(c => 
