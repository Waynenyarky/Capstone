...
const DEFAULT_EPOCH_WINDOW_MS = Number(process.env.DIGEST_EPOCH_WINDOW_MS) || 15 * 60 * 1000;
...
async function flushEpoch() {
  ...
  try {
    ...
    logger.info(`[DigestService] Created digest ${digest._id} with ${leafHashes.length} leaves`);
    return digest;
  } catch (err) {
...
async function markFailed(digestId, error) {
  ...
  if (newRetries >= maxRetries) {
    ...
    await AuditLog.updateMany(
      { _id: { $in: digest.auditLogIds } },
      { $set: { blockchainStatus: 'failed', blockchainError: 'Digest anchor failed' } }
    );

    logger.error(`[DigestService] Digest ${digestId} failed after ${maxRetries} retries: ${error}`);
  } else {
    await AuditDigest.findByIdAndUpdate(digestId, {
      $set: { anchorStatus: 'retrying', anchorError: error },
      $inc: { anchorRetries: 1 },
    });
    logger.warn(`[DigestService] Digest ${digestId} retry ${newRetries}/${maxRetries}: ${error}`);
  }
