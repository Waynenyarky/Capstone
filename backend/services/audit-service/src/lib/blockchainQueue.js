...
const batchBuffer = [];
const BATCH_FLUSH_INTERVAL_MS =
  Number(process.env.BATCH_FLUSH_INTERVAL_MS) || 60000; // default 60s
let batchFlushTimer = null;

// Digest anchor timer for mainnet_budget mode
let digestAnchorTimer = null;
const DIGEST_ANCHOR_INTERVAL_MS =
  Number(process.env.DIGEST_ANCHOR_INTERVAL_MS) || 30000; // 30s check
  ...
  while (queue.length > 0 && !shouldStop) {
    ...
    try {
      ...
      switch (item.operation) {
        case "logAuditHash":
          result = await blockchainService.logAuditHash(
            item.args[0],
            item.args[1],
          );
          break;
        case "logCriticalEvent":
          ...
          result = await blockchainService.logCriticalEventAdaptive(
            item.args[0],
            item.args[1],
            item.args[2],
          );
      ...
      if (result.success) {
        if (item.auditLogId) {
          const update = {
            ...
            blockNumber:
              result.blockNumber != null ? Number(result.blockNumber) : null,
            blockchainStatus: "anchored",
          };
          await AuditLog.findByIdAndUpdate(item.auditLogId, update).catch(
            (err) => {
              console.error(
                "Failed to update audit log with blockchain data:",
                err,
              );
            },
          );
        ...
        if (item.retries < MAX_RETRIES && !shouldStop) {
          ...
          console.error(
            `Blockchain operation failed after ${MAX_RETRIES} retries:`,
            {
...
async function flushBatchBuffer() {
  ...
  const digestInput = items
    .map((item) => {
      if (
        item.operation === "logAuditHash" &&
  ...
  console.log(
    `[BlockchainQueue] Flushed batch: ${items.length} events → 1 digest tx (${digestHash.slice(0, 16)}...)`,
  );
}
  ...
  for (const digest of pendingDigests) {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(
          `[BlockchainQueue] Digest ${digest._id} anchored: ${result.txHash}`,
        );
      } else {
      ...
      console.error(
        `[BlockchainQueue] Error anchoring digest ${digest._id}:`,
        err,
      );
