...
class DatabaseOptimizationService {
  ...
  async initialize() {
    try {
      ...
      console.error(
        "Failed to initialize database optimization service:",
        error,
      );
    }
  }
  ...
  applyQueryOptimizations(query) {
    ...
    Object.keys(optimized).forEach((key) => {
      if (optimized[key] instanceof RegExp) {
        const flags = optimized[key].flags.includes("i")
          ? optimized[key].flags
          : optimized[key].flags + "i";
  ...
  shouldUseAggregation(query, options) {
    ...
    const hasComplexOperations =
      options.sort && Object.keys(options.sort).length > 1;
  ...
  async suggestIndexes(collection, sampleQueries = []) {
    try {
      ...
      for (const index of commonIndexes) {
        const exists = analysis.indexes.some(
          (existing) => existing.name === index.options.name,
        );
        if (!exists) {
  ...
  async archiveOldData(collection, cutoffDate, archiveCollection) {
    try {
      ...
      const docsToArchive = await source
        .find({
      ...
      if (docsToArchive.length > 0) {
        ...
        console.log(
          `Archived ${docsToArchive.length} documents from ${collection}`,
        );
      }
