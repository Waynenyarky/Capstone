...
export default function CessationOverviewPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      const res = await get('/api/lgu-manager/overview/cessations')
  ...
  const kpiData = useMemo(() => ({
    activeCessations: data?.activeCessations ?? 0,
    temporary: data?.temporary ?? 0,
    permanent: data?.permanent ?? 0,
    underAppeal: data?.underAppeal ?? 0,
    overdueUnresolved: data?.overdueUnresolved ?? 0,
    liftedResolved: data?.liftedResolved ?? 0
  }), [data])
  ...
  const slaData = useMemo(() => ({
    ...
    repeatCessationCount: data?.repeatCessationCount ?? '—',
    complianceResolutionRate: data?.complianceResolutionRate ?? '—'
  }), [data])
  ...
  const handleExport = useCallback(() => {
    const headers = ['Cessation ID', 'Business ID', 'Type', 'Issuance Date', 'Status', 'Duration', 'Officer', 'Compliance Status']
    const rows = (tableData || []).map(item => [
      item.id, item.businessId, item.type, item.issuanceDate, item.status, item.duration, item.officer, item.complianceStatus
    ])
    ...
    a.href = url
    a.download = `cessation-overview-${Date.now()}.csv`
    a.click()
            ...
            <Tabs defaultActiveKey="active">
              ...
              >
                <Row gutter={[16, 16]}>
                  ...
                  <Col xs={24} sm={8}>
                    <Statistic title="Average Duration" value={statusBreakdown?.active?.avgDays ?? '—'} suffix="days" />
                  </Col>
                  <Col xs={24} sm={8}>
                    <Statistic title="Percentage of Total" value={statusBreakdown?.active?.percentage ?? '—'} suffix="%" />
                  </Col>
                </Row>
              </TabPane>
              <TabPane 
              ...
              >
                <Row gutter={[16, 16]}>
                  ...
                  <Col xs={24} sm={8}>
                    <Statistic title="Average Duration" value={statusBreakdown?.lifted?.avgDays ?? '—'} suffix="days" />
                  </Col>
                  <Col xs={24} sm={8}>
                    <Statistic title="Percentage of Total" value={statusBreakdown?.lifted?.percentage ?? '—'} suffix="%" />
                  </Col>
                </Row>
              </TabPane>
              <TabPane 
              ...
              >
                <Row gutter={[16, 16]}>
                  ...
                  <Col xs={24} sm={8}>
                    <Statistic title="Average Duration" value={statusBreakdown?.underAppeal?.avgDays ?? '—'} suffix="days" />
                  </Col>
                  <Col xs={24} sm={8}>
                    <Statistic title="Percentage of Total" value={statusBreakdown?.underAppeal?.percentage ?? '—'} suffix="%" />
                  </Col>
                </Row>
              </TabPane>
              <TabPane 
              ...
              >
                <Row gutter={[16, 16]}>
                  ...
                  <Col xs={24} sm={8}>
                    <Statistic title="Average Duration" value={statusBreakdown?.overdue?.avgDays ?? '—'} suffix="days" />
                  </Col>
                  <Col xs={24} sm={8}>
              ...
              <Col xs={24} sm={12} md={6}>
                <Card style={{ textAlign: 'center', border: `1px solid ${token.colorBorderSecondary}` }}>
                  <Statistic
                    ...
                    value={slaData.repeatCessationCount}
                    suffix="cases"
                    valueStyle={{ fontSize: 20, fontWeight: 600 }}
                  />
                </Card>
              ...
              <Col xs={24} sm={12} md={6}>
                <Card style={{ textAlign: 'center', border: `1px solid ${token.colorBorderSecondary}` }}>
                  <Statistic
                    ...
                    value={slaData.complianceResolutionRate}
                    suffix="%"
          ...
          <Space direction="vertical" style={{ width: '100%' }} size="middle">
            ...
            <Row gutter={[16, 16]}>
              <Col xs={24} md={12}>
                <Card size="small" title="Top Violation Categories" style={{ background: token.colorBgContainer }}>
                  {(data?.topViolationCategories || []).length > 0 ? (
                    <Space direction="vertical" style={{ width: '100%' }} size="small">
                      {data.topViolationCategories.map((item, idx) => (
                        <div key={idx}>
                    ...
                    <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No violation category data available" />
                  )}
                </Card>
              </Col>
              <Col xs={24} md={12}>
                <Card size="small" title="Legal Basis References (Aggregated)" style={{ background: token.colorBgContainer }}>
                  {(data?.legalBasisReferences || []).length > 0 ? (
                    <Space direction="vertical" style={{ width: '100%' }} size="small">
                      {data.legalBasisReferences.map((item, idx) => (
                        <div key={idx}>
                          <Text strong>{item.label}</Text>
                          <br />
                          <Text type="secondary" style={{ fontSize: 12 }}>{item.detail}</Text>
                    ...
                    <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No legal basis data available" />
                  )}
                </Card>
              </Col>
            </Row>
            <Card size="small" title="Repeat Violation Patterns" style={{ background: token.colorBgContainer }}>
              <Text type="secondary" style={{ fontSize: 13 }}>
                {data?.repeatViolationSummary || 'No repeat violation pattern data available.'}
              </Text>
            </Card>
          </Space>
        </Card>
              ...
              <Col xs={24} md={8}>
                <Card size="small" style={{ background: token.colorBgContainer }}>
                  <Space direction="vertical" size="small" style={{ width: '100%' }}>
                    ...
                    <Text type="secondary" style={{ fontSize: 12 }}>
                      {data?.officerPerformance?.cessationsIssued || 'No data available'}
                    </Text>
                  </Space>
                </Card>
              </Col>
              ...
              <Col xs={24} md={8}>
                <Card size="small" style={{ background: token.colorBgContainer }}>
                  <Space direction="vertical" size="small" style={{ width: '100%' }}>
                    ...
                    <Text type="secondary" style={{ fontSize: 12 }}>
                      {data?.officerPerformance?.departmentDistribution || 'No data available'}
                    </Text>
                  </Space>
                </Card>
              </Col>
