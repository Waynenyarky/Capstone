...
export default function AppealsOverviewPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      const res = await get('/api/lgu-manager/overview/appeals')
  ...
  const kpiData = useMemo(() => ({
    ...
    approvedAppeals: data?.approvedAppeals ?? 0,
    rejectedAppeals: data?.rejectedAppeals ?? 0,
  ...
  const handleExport = useCallback(() => {
    const headers = ['Appeal ID', 'Appeal Type', 'Business ID', 'Submission Date', 'Status', 'Reviewer', 'Duration', 'SLA Status']
    const rows = (tableData || []).map(item => [
      item.id, item.type, item.businessId, item.submissionDate, item.status, item.reviewer, item.duration, item.slaStatus
    ])
    ...
    a.href = url
    a.download = `appeals-overview-${Date.now()}.csv`
    a.click()
            ...
            <Col xs={24} sm={12} md={8} lg={4}>
              ...
              >
                <Statistic
                  ...
                  prefix={<CheckOutlined style={{ color: token.colorSuccess }} />}
                  valueStyle={{ 
            ...
            <Tabs defaultActiveKey="pending">
              ...
              >
                <Row gutter={[16, 16]}>
                  ...
                  <Col xs={24} sm={8}>
                    <Statistic title="Average Time in Status" value={statusBreakdown?.underReview?.avgDays ?? '—'} suffix="days" />
                  </Col>
                  <Col xs={24} sm={8}>
                    <Statistic title="Percentage of Total" value={statusBreakdown?.underReview?.percentage ?? '—'} suffix="%" />
                  </Col>
                </Row>
              </TabPane>
              <TabPane 
              ...
              >
                <Row gutter={[16, 16]}>
                  ...
                  <Col xs={24} sm={8}>
                    <Statistic title="Average Time in Status" value={statusBreakdown?.approved?.avgDays ?? '—'} suffix="days" />
                  </Col>
                  <Col xs={24} sm={8}>
              ...
              >
                <Row gutter={[16, 16]}>
                  ...
                  <Col xs={24} sm={8}>
                    <Statistic title="Average Time in Status" value={statusBreakdown?.rejected?.avgDays ?? '—'} suffix="days" />
                  </Col>
                  <Col xs={24} sm={8}>
              ...
              <TabPane tab="Escalated / Delayed" key="escalated">
                <Row gutter={[16, 16]}>
                  <Col xs={24} sm={8}>
                    <Statistic title="Count" value={statusBreakdown?.escalated?.count ?? 0} />
                  </Col>
                  <Col xs={24} sm={8}>
                    <Statistic title="Average Time in Status" value={statusBreakdown?.escalated?.avgDays ?? '—'} suffix="days" />
                  </Col>
                  <Col xs={24} sm={8}>
                    <Statistic title="Percentage of Total" value={statusBreakdown?.escalated?.percentage ?? '—'} suffix="%" />
                  </Col>
                </Row>
              </TabPane>
            </Tabs>
            ...
            <Row gutter={[16, 16]}>
              <Col xs={24} md={12}>
                <Card size="small" title="Repeat Appeals from Same Business (Masked)" style={{ background: token.colorBgContainer }}>
                  {(data?.repeatAppeals || []).length > 0 ? (
                    <Space direction="vertical" style={{ width: '100%' }} size="small">
                      {data.repeatAppeals.map((item, idx) => (
                        <div key={idx}>
                    ...
                    <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No repeat appeal data available" />
                  )}
                </Card>
              </Col>
              <Col xs={24} md={12}>
                <Card size="small" title="Appeal Frequency by Category" style={{ background: token.colorBgContainer }}>
                  {(data?.appealsByCategory || []).length > 0 ? (
                    <Space direction="vertical" style={{ width: '100%' }} size="small">
                      {data.appealsByCategory.map((item, idx) => (
                        <div key={idx}>
                          <Text strong>{item.label}</Text>
                          <br />
                          <Text type="secondary" style={{ fontSize: 12 }}>{item.detail}</Text>
                    ...
                    <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No category data available" />
                  )}
                </Card>
              </Col>
            </Row>
            <Row gutter={[16, 16]}>
              <Col xs={24} md={12}>
                <Card size="small" title="Appeals Linked to High-Risk Violations or Cessations" style={{ background: token.colorBgContainer }}>
                  {(data?.highRiskAppeals || []).length > 0 ? (
                    <Space direction="vertical" style={{ width: '100%' }} size="small">
                      {data.highRiskAppeals.map((item, idx) => (
                        <div key={idx}>
                          <Text strong>{item.label}</Text>
                          <br />
                          <Text type="secondary" style={{ fontSize: 12 }}>{item.detail}</Text>
                    ...
                    <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No high-risk appeal data available" />
                  )}
                </Card>
              </Col>
              <Col xs={24} md={12}>
                <Card size="small" title="AI-Assisted Indicators (Advisory)" style={{ background: token.colorBgContainer }}>
                  {(data?.aiIndicators || []).length > 0 ? (
                    <Space direction="vertical" style={{ width: '100%' }} size="small">
                      {data.aiIndicators.map((item, idx) => (
                        <div key={idx}>
                          <Badge status={item.severity || 'default'} text={<Text>{item.message}</Text>} />
                        </div>
                      ))}
                    </Space>
                  ) : (
                    <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No AI indicators available" />
                  )}
                </Card>
              </Col>
            </Row>
              ...
              <Col xs={24} md={8}>
                <Card size="small" style={{ background: token.colorBgContainer }}>
                  <Space direction="vertical" size="small" style={{ width: '100%' }}>
                    ...
                    <Text type="secondary" style={{ fontSize: 12 }}>
                      {data?.reviewerPerformance?.appealsHandled || 'No data available'}
                    </Text>
                  </Space>
                </Card>
              </Col>
              <Col xs={24} md={8}>
                <Card size="small" style={{ background: token.colorBgContainer }}>
                  <Space direction="vertical" size="small" style={{ width: '100%' }}>
                    ...
                    <Text type="secondary" style={{ fontSize: 12 }}>
                      {data?.reviewerPerformance?.avgDecisionTime || 'No data available'}
                    </Text>
                  </Space>
                </Card>
              </Col>
              <Col xs={24} md={8}>
                <Card size="small" style={{ background: token.colorBgContainer }}>
                  <Space direction="vertical" size="small" style={{ width: '100%' }}>
                    ...
                    <Text type="secondary" style={{ fontSize: 12 }}>
                      {data?.reviewerPerformance?.slaCompliance || 'No data available'}
                    </Text>
                  </Space>
                </Card>
              </Col>
