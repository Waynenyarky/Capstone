...
import { recommendLineOfBusiness, submitLobFeedback } from '../../../services/aiService.js'
import {
  normalizeActivityFromForm,
  normalizeRecommendation,
  getDetailedLinesForTaxCode,
} from '../../../utils/lobUtils.js'
import { LINE_OF_BUSINESS } from '@/constants/lineOfBusiness.js'
import ManualAddModal from './modals/ApplicationManualAddModal'
...
export default function ApplicationAiLobRecommendation({ field, form, readOnly = false, formDataKey = null }) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const res = await recommendLineOfBusiness(description, { timeoutMs: 10 * 60 * 1000 })
  ...
  const sendLobFeedback = useCallback(async (businessDesc, recommendationsWithAccepted) => {
    if (!recommendationsWithAccepted?.length) return
    await submitLobFeedback({
  ...
  const detailedLineOptions = getDetailedLinesForTaxCode(manualTaxCode)
  ...
  return (
    <div>
            ...
            {aiRecommendations.map((rec, idx) => {
              ...
              <Card
                ...
                style={{
                  ...
                  background: isConfirmed ? token.colorSuccessBg : token.colorBgContainer,
                }}
              >
      ...
      <ManualAddModal
        ...
        manualDetailedLine={manualDetailedLine}
        onTaxCodeChange={setManualTaxCode}
        onDetailedLineChange={setManualDetailedLine}
        detailedLineOptions={detailedLineOptions}
      />
    </div>
  )
}
