...
vi.mock('../../services/staffService', () => ({
  getAdminList: vi.fn(() => Promise.resolve([
    ...
    { _id: 'admin2', email: 'admin2@example.com' }
  ]))
}))
...
describe('useMaintenanceApprovalActions', () => {
  ...
  it('handles approve click', () => {
    ...
    expect(result.current.actionApproved).toBe(true)
    expect(result.current.comment).toBe('')
    expect(result.current.actionModalOpen).toBe(true)
  })
  ...
  it('handles deny click', () => {
    ...
    expect(result.current.actionApproved).toBe(false)
    expect(result.current.comment).toBe('')
    expect(result.current.actionModalOpen).toBe(true)
  })
  ...
  it('submits deny action with comment', async () => {
    ...
    act(() => {
      result.current.handleDenyClick()
      result.current.setComment('Reject reason')
    })
    ...
    expect(mockOnApprove).toHaveBeenCalledWith('approval1', false, 'Reject reason')
  })
  ...
  it('detects overlapping maintenance', () => {
    ...
    const overlappingApproval = {
      approvalId: 'approval2',
      status: 'approved',
      requestType: 'maintenance_mode',
      requestDetails: {
        scheduledStartAt: dayjs().add(1, 'day').add(1, 'hour').toISOString(),
        expectedResumeAt: dayjs().add(1, 'day').add(3, 'hour').toISOString()
      }
    }

    const { result } = renderHook(() =>
      useMaintenanceApprovalActions(approvalWithOverlap, [approvalWithOverlap, overlappingApproval], mockOnApprove, mockOnUndoVote, mockOnCancelApproved, mockOnRefresh)
    )

    expect(result.current.hasOverlappingMaintenance()).toBe(true)
  })
