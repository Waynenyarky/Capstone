...
export default function AuditTamperDesktopView({
  ...
  const selectedLabel = AUDIT_TAMPER_NAV_ITEMS.find((i) => i.key === tabKey)?.label ?? tabKey
  const SelectedIcon = AUDIT_TAMPER_NAV_ITEMS.find((i) => i.key === tabKey)?.icon
    ...
    >
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {AUDIT_TAMPER_NAV_ITEMS.map((item) => renderNavItem(item, tabKey === item.key))}
        </div>
      </div>
