...
export default function OfficerAppeals({ officerData, onReviewComplete }) {
  const [selectedItem, setSelectedItem] = useState(null)
  const listPanel = useOfficerListPanel({ activeTab: 'appeals' })

  const getItemId = useCallback((item) => {
    return item._id || item.appealId
  }, [])

  const filteredList = useMemo(() => {
    let list = officerData?.appeals || []
  ...
  }, [officerData?.appeals, listPanel.statusFilter, listPanel.search])

  const handleItemClick = useCallback((item) => {
    const id = getItemId(item)
    setSelectedItem({ ...item, _itemType: 'appeals', _itemId: id })
  }, [getItemId])
  ...
  return (
    <Splitter style={{ height: '100%' }}>
      <Splitter.Panel min="25%" defaultSize="25%" style={{ overflow: 'hidden' }}>
        <OfficerListPanel
          items={filteredList}
          isLoading={officerData?.loadingMap?.appeals}
      ...
      <Splitter.Panel min="50%" defaultSize="75%" style={{ overflow: 'hidden' }}>
        {!selectedItem ? (
          <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Empty description="Select an appeal to view details" />
          </div>
        ) : (
          <AppealDetailPanel appeal={selectedItem} onReviewComplete={onReviewComplete} />
        )}
      </Splitter.Panel>
    </Splitter>
  )
