...
import { deleteAccountStart } from '@/features/authentication/services/authService'
import { useNotifier } from '@/shared/notifications'
import useCooldown from '@/features/authentication/hooks/useCooldown'

export function useResendDeleteAccountCode({ email, cooldownSec = 60, onSent } = {}) {
  ...
  const handleResend = useCallback(async () => {
    ...
    try {
      setSending(true)
      const data = await deleteAccountStart({ email })
      ...
      console.error('Resend delete account code error:', err)
      try {
...
export default useResendDeleteAccountCode
