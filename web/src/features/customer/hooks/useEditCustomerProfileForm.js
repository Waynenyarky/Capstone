...
import { getCustomerProfile, updateCustomerProfile } from "@/features/customer/services"
import { useNotifier } from '@/shared/notifications.js'

export function useEditCustomerProfileForm({ onSubmit } = {}) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const data = await getCustomerProfile(currentUser, role)
      form.setFieldsValue({
  ...
  const handleFinish = async (values) => {
    try {
      setSubmitting(true)
      const data = await updateCustomerProfile(values, currentUser, role)
      ...
      // Map common backend validation errors to specific fields
