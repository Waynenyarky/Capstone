...
describe('NotificationService - Core Functionality', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  describe('Complex Scenarios', () => {
    ...
    it('should handle real-time subscription workflow', async () => {
      // Subscribe to notifications
      const subscribeResponse = { success: true, subscriptionId: 'sub-123' }
      post.mockResolvedValue(subscribeResponse)

      const subscribed = await subscribeToNotifications('business-123', 'socket-456')
      expect(subscribed.success).toBe(true)

      // Get notifications
      const notificationsResponse = {
        notifications: [
          { id: 'notif-1', title: 'Real-time Test', message: 'Test message' }
        ]
      }
      get.mockResolvedValue(notificationsResponse)
      ...
      // Unsubscribe
      const unsubscribeResponse = { success: true }
      post.mockResolvedValue(unsubscribeResponse)

      const unsubscribed = await unsubscribeFromNotifications('business-123', 'socket-456')
