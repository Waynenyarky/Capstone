...
// We'll mock the auth service loginStart and the cooldown hook so we can
...
describe('Resend cooldown & lock handling', () => {
  beforeEach(() => {
    startSpy.mockClear()
    loginStart.mockReset()
  })

  it('starts cooldown using retryAfter from successful loginStart', async () => {
    loginStart.mockResolvedValueOnce({ devCode: '123456', retryAfter: 90 })
  ...
  it('starts cooldown using lockedUntil from loginStart error body', async () => {
    ...
    loginStart.mockRejectedValueOnce({ body: { lockedUntil: future } })
